	...
	function cekinput(fid) {  
	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='departemen_id') {
		  ...
		  if (document.getElementById(arrf[i]).name=='tanggal_ttd') {
			alert('Titimangsa tidak boleh kosong!');				
		  }
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				include("app/exec/insert_setup_periode_raport_p5.php");
				
				$aktif = "checked";
				
				if ($ref != "") {
					$sql=$select->list_setup_periode_raport_p5($ref);
					$setup_periode_raport_data=$sql->fetch(PDO::FETCH_OBJ);	
					
					$departemen_id = $setup_periode_raport_data->departemen_id;
					$departemen_sekolah_id = $setup_periode_raport_data->departemen_sekolah_id;
					$tingkat_id = $setup_periode_raport_data->tingkat_id;
					$tanggal_ttd = date("d-m-Y", strtotime($setup_periode_raport_data->tanggal_ttd));
					if($tanggal_ttd == "01-01-1970") {
						$tanggal_ttd = "";
					}
					
					$aktif = "";
					if ($setup_periode_raport_data->aktif == 1) {
						$aktif = "checked";
					} else {
						$aktif = "";
					}
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="setup_periode_raport2" id="setup_periode_raport2" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('departemen_id,departemen_sekolah_id,idtahunajaran,semester_id,tingkat_id,tanggal_ttd');" >
            	...
            	<div class="form-group">
					...
					<div class="col-sm-3">
						<select name="departemen_id" id="departemen_id" class="chosen-select form-control" onchange="loadHTMLPost2('<?php echo $__folder ?>app/raport_p5_ajax.php','sekolah_id','getsekolah_tingkat_setup5','departemen_id','__folder')" >
							<option value=""></option>
							<?php select_departemen_id($departemen_id); ?>
						</select>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-4" id="sekolah_id">
						<select name="departemen_sekolah_id" id="departemen_sekolah_id" class="chosen-select form-control" onchange="loadHTMLPost2('<?php echo $__folder ?>app/raport_p5_ajax.php','id_tingkat','getkelas_tingkat_sekolah_setup5','departemen_sekolah_id','__folder')">
							<option value=""></option>
							<?php select_sekolah_unit_id($departemen_id, $departemen_sekolah_id); ?>
						</select>
					</div>
				</div>
            	...
            	<div class="form-group">
					...
					<div class="col-lg-2">
						<select name="idtahunajaran" id="idtahunajaran" class="chosen-select form-control" />
							<option value=""></option>
							<?php select_thnajaran($setup_periode_raport_data->idtahunajaran); ?>
						</select>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-lg-2">
						<select name="semester_id" id="semester_id" class="chosen-select form-control" >
							<option value=""></option>
							<?php 
								select_semester_all("SMA", $setup_periode_raport_data->semester_id); 
							?>
						</select>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-3" id="id_tingkat">
						<select name="tingkat_id" id="tingkat_id" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_tingkat_sekolah($setup_periode_raport_data->departemen_sekolah_id, $tingkat_id); ?>
						</select>								
					</div>
				</div>
				
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'Period Date'; } else { echo 'Titimangsa'; } ?> *)</label>
					<div class="col-sm-3">
						<input type="text" id="tanggal_ttd" name="tanggal_ttd" style="font-size: 12px" class="form-control date-picker" data-date-format="dd-mm-yyyy" autocomplete="off" value="<?php echo $tanggal_ttd ?>">								
					</div>
				</div>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowupd('frmsetup_periode_raport_p5')==1) { ?>
                            <?php if($ref!='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Update" />
    						<?php } ?>
                        <?php } ?>
						
                        <?php if (allowadd('frmsetup_periode_raport_p5')==1) { ?>
    						<?php if($ref=='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Simpan" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php if (allowdel('frmsetup_periode_raport_p5')==1) { ?>
                            &nbsp;
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('setup_periode_raport_p5_view') ?>'" />
