...
<script language="javascript">
	function cekinput(fid) {  
	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='kode') {
			alert('Kode UKBM tidak boleh kosong!');				
		  }
	  ...
	  var jumlah_ukbm = document.getElementById('jumlah_ukbm').value;
	  jumlah_ukbm = jumlah_ukbm.replace(/[^\d-.]/g,"");
	  
	  if(jumlah_ukbm == "" || jumlah_ukbm == 0) {
	  	alert("Jumlah Kehadiran harus diisi !");
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				include("app/exec/insert_ukbm.php");
				
				$ekstra_sifat = "";
				$aktif = "checked";
				if ($ref != "") {
					$sql=$select->list_ukbm($ref);
					$ukbm_data=$sql->fetch(PDO::FETCH_OBJ);	
					
					$aktif = "";
					if ($ukbm_data->aktif == 1) {
						$aktif = "checked";
					} else {
						$aktif = "";
					}
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="ukbm2" id="ukbm2" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('idtingkat,idsemester,idpelajaran,idguru,kode,deskripsi');" >
                ...
                <div class="form-group">
					...
					<div class="col-sm-3">
						<select name="idtingkat" id="idtingkat" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_tingkat($ukbm_data->idtingkat) ?>
						</select>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<select name="idsemester" id="idsemester" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_semester_all("SMA", $ukbm_data->idsemester) ?>
						</select>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-4">						
						<select name="idpelajaran" id="idpelajaran" class="chosen-select form-control" >
							<option value=""></option>
							<?php 
								//select_pelajaran_all($ukbm_data->idpelajaran) 
								select_pelajaran_krs($ukbm_data->idpelajaran)
							?>
						</select>						
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-5">						
						<select name="idguru" id="idguru" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_petugas($ukbm_data->idguru) ?>
						</select>						
					</div>
				</div>
				
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">RPP</label>
					<div class="col-sm-6">						
						<select name="idrpp" id="idrpp" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_where("rpp", "replid", "koderpp", "aktif", "1", $ukbm_data->idrpp) ?>
						</select>						
					</div>
				</div>
				
                <div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Kode PAT *)</label>
					<div class="col-sm-3">
						<input type="text" name="kode" id="kode" class="form-control" value="<?php echo $ukbm_data->kode ?>">
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-5">
						<textarea id="deskripsi" name="deskripsi" class="autosize-transition form-control"><?php echo $ukbm_data->deskripsi ?></textarea>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<input type="file" id="file_ukbm" name="file_ukbm" />
                        <br />
        				<?php /*if (!empty($ukbm_data->file_ukbm)) { ?>
        					<img src="app/file_ukbm/<?php echo $ukbm_data->file_ukbm; ?>" width="150" height="150" />
        				<?php }*/ ?>
                        <input size="25" type="hidden" id="file_ukbm2" name="file_ukbm2" value="<?php echo $ukbm_data->file_ukbm; ?>">  
                        
                        <?php if($ukbm_data->file_ukbm != "") { ?>
							<a class="label label-success" href="../app/ukbm_download.php?ref=<?php echo $ref ?>" target="_blank" title="Download"><?php echo $ukbm_data->file_ukbm; ?>
							</a>
							
							<!--<a style="font-weight: bold;" href="javascript:void(0);" name="Find" title="Download" onClick=window.open("<?php echo $__folder ?>app/ukbm_download.php?ref=<?php echo $ref ?>","Find","width=1200,height=600,left=50,top=20,toolbar=0,status=0,scroll=1,scrollbars=no"); >
								<?php echo $ukbm_data->info3 ?>
							</a>-->
						<?php } ?>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<input type="text" name="jumlah_ukbm" id="jumlah_ukbm" onkeyup="formatangka('jumlah_ukbm')" class="form-control" value="<?php echo $ukbm_data->jumlah_ukbm ?>">
					</div>
				</div>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowupd('frmukbm')==1) { ?>
                            <?php if($ref!='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Update" />
    						<?php } ?>
                        <?php } ?>
						
                        <?php if (allowadd('frmukbm')==1) { ?>
    						<?php if($ref=='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Simpan" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php if (allowdel('frmukbm')==1) { ?>
                            &nbsp;
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('ukbm_view') ?>'" />
