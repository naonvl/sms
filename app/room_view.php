...
<script type="text/javascript">
	function hapus(id) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "<?php echo obraxabrix(room_view) ?>/xm8r389xemx23xb2378e23/"+id+" ";
		}
	}
	
	function room_item(ref) {
		window.open("app/room_item.php?ref="+ref,"Find","width=1000,height=620,left=50,top=10,toolbar=0,status=0,scroll=1,scrollbars=no");
	}
</script>
...
<div class="page-content">			
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<form class="form-horizontal" role="form" action="" method="post" name="room_view" id="room_view" class="form-horizontal" enctype="multipart/form-data" >
            	
            	<div class="form-group">
                    <label class="col-sm-3 control-label no-padding-right" for="form-field-1">Nama Gedung</label>
                    
                    <div class="col-sm-3">
                    	 <select id="asset_ref" name="asset_ref" class="chosen-select form-control" onChange="loadHTMLPost2('<?php echo $__folder ?>app/room_ajax.php','room_filter','getdata','asset_ref')" style="max-width: 300px; font-size: 12px;" >
                    	 	<option value=""></option>
                          	<?php 
                                select_asset_gedung($asset_ref) 
                            ?>	
            	...
            	<div class="form-group">
                    <label class="col-sm-3 control-label no-padding-right" for="form-field-1">ID Ruang</label>
                    
                    <div class="col-sm-5" id="room_filter">
				...
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'Room Code'; } else { echo 'Kode Ruang'; } ?></label>
					<div class="col-sm-3">
            	...
            	<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'Room Name'; } else { echo 'Nama Ruang'; } ?></label>
					<div class="col-sm-3">
		...
		<div class="col-xs-12">
            ...
            <?php
                ...
                if ($delete == "xm8r389xemx23xb2378e23") {
					...
					$delete2=new delete;
					$delete2->delete_room($segmen4);
			?>
					<div class="alert alert-success">
						<strong>Delete Data successfully</strong>
					</div>
                    
                    <meta http-equiv="Refresh" content="0;url=<?php echo $nama_folder . '/' . obraxabrix(room_view); ?>" />
			<?php
					...
					<div>
						<table id="dynamic-table" class="table table-striped table-bordered table-hover">
							...
							<tbody>
                                <?php			
									$i = 0;
            						$sql=$select->list_room($ref, $code, $name, $all, $asset_ref);
						            while($row_room=$sql->fetch(PDO::FETCH_OBJ)){
								...
								?>
                                        ...
                                        <tr>
                                            <td><?php echo $i ?></td>
                                            <td><?php echo $row_room->code ?></td>
					                    	<td><?php echo $row_room->name ?></td>
					                    	<td><?php echo $row_room->employee_name ?></td>
					                    	<td align="center">
												<?php if ($row_room->booking == 1) { ?>
													<span class="red">
											...
											<td>
                                                ...
                                                <?php if (allowupd('frmroom')==1) { ?>
    												<a href="<?php echo obraxabrix('room') ?>/<?php echo $row_room->ref ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
    													<span class="green">
                                                ...
                                                <?php if (allowdel('frmroom')==1) { ?>    
                                                    &nbsp;
    												<a href="JavaScript:hapus('<?php echo $row_room->ref ?>')" class="tooltip-error" data-rel="tooltip" title="Delete">
    													<span class="red">
