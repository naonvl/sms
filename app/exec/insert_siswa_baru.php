...
<?php
	//} else {		
		...
		//$ref=notran($tanggal, 'frmsiswa_baru', '', '', ''); //---get no ref
		
		$hs=$insert->insert_siswa_baru($ref);
	...
	?>
			...
			<script>
				window.location = '<?php echo $__folder ?><?php echo obraxabrix(siswa_baru) ?>/<?php echo $hs ?>';
...
<?php	
	//} else {	
		$hs=$update->update_siswa_baru($_POST['replid']);
