	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				include("app/exec/insert_siswa_izin.php");
				
				$tanggal = date("d-m-Y");
				
				if ($ref != "") {
					$sql=$select->list_siswa_izin($ref);
					$siswa_izin_data=$sql->fetch(PDO::FETCH_OBJ);	
					
					$tanggal = date("d-m-Y", strtotime($siswa_izin_data->tanggal));		
					$departemen = $siswa_izin_data->departemen;			
				}		
			?>
            
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="siswa_izin2" id="siswa_izin2" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('tanggal,idsiswa,idtingkat,idkelas');" >
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<select name="departemen" id="departemen" class="chosen-select form-control" onchange="loadHTMLPost2('<?php echo $__folder ?>app/siswa_ajax.php','tingkat_id','gettingkat_list','departemen')" >
							<option value=""></option>
							<?php select_departemen($siswa_izin_data->departemen); ?>
						</select>
					</div>
				</div>
                ...
                <div class="form-group">
					...
					<div class="col-sm-5">						
						<select name="idsiswa" id="idsiswa" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_siswa_id($siswa_izin_data->idsiswa) ?>
						</select>						
					</div>
				</div>
                ...
                <div class="form-group">
					...
					<div class="col-sm-3" id="tingkat_id">
						<select name="idtingkat" id="idtingkat" class="chosen-select form-control" onchange="loadHTMLPost3('<?php echo $__folder ?>app/presensi_ukbm_ajax.php','kelas_id','getkelas','idtingkat')" >
							<option value=""></option>
							<?php select_tingkat_unit($departemen, $siswa_izin_data->idtingkat); ?>
						</select>			
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-3" id="kelas_id">
						<select name="idkelas" id="idkelas" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_kelas($siswa_izin_data->idtingkat, $siswa_izin_data->idkelas); ?>
						</select>								
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-5">
						<textarea id="alasan" name="alasan" class="form-control"><?php echo $siswa_izin_data->alasan ?></textarea>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-5">
						<textarea id="penanganan" name="penanganan" class="form-control"><?php echo $siswa_izin_data->penanganan ?></textarea>
					</div>
				</div>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowadd('frmsiswa_izin')==1) { ?>
    						<?php if($ref=='') { ?>
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('siswa_izin_view') ?>'" />
