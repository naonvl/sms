	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				$ref2 = notran(date('y-m-d'), 'frmsurat_keluar', '', '', ''); 
					
				include("app/exec/insert_surat_keluar.php");
				
				$tanggal = date("d-m-Y");
				if ($ref != "") {
					$sql=$select->list_surat_keluar($ref);
					$row_surat_keluar=$sql->fetch(PDO::FETCH_OBJ);	
					
					$ref2		=	$row_surat_keluar->ref;
					$tanggal	=	date("d-m-Y", strtotime($row_surat_keluar->tanggal));
					
				}		
			?>
            
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="surat_keluar_ukbm" id="surat_keluar_ukbm" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('ref,tanggal,idkelompok_surat,petugas,tujuan');" >
            	...
            	<div class="row">
					<div class="col-sm-6">
						<div class="row">
							...
							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'Document No.'; } else { echo 'Nomor Surat'; } ?> *)</label>
								<div class="col-sm-5">
									<input type="text" id="no_surat" name="no_surat" class="form-control" value="<?php echo $row_surat_keluar->no_surat ?>"> 
								</div>
							</div>
			                ...
			                <div class="form-group">
								...
								<div class="col-sm-5">
									<select name="idkelompok_surat" id="idkelompok_surat" class="chosen-select form-control" >
										<option value=""></option>
										<?php populate_select("kelompok_surat","replid","nama",$row_surat_keluar->idkelompok_surat); ?>
									</select>
								</div>
							</div>
							
							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'PIC'; } else { echo 'Nama Petugas'; } ?> *)</label>
								<div class="col-sm-5">
									<input type="text" id="petugas" name="petugas" class="form-control" value="<?php echo $row_surat_keluar->petugas ?>"> 
								</div>
							</div>
					...
					<div class="col-sm-6">
						<div class="row">
							...
							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'Destination'; } else { echo 'Ditujukan Kepada'; } ?> *)</label>
								<div class="col-sm-5">
									<input type="text" id="tujuan" name="tujuan" class="form-control" value="<?php echo $row_surat_keluar->tujuan ?>"> 
								</div>
							</div>
							
							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Upload File Dokumen</label>
								<div class="col-sm-5">
									<input type="file" id="file_dokumen" name="file_dokumen" />
			                        <br />
			        				<?php if (!empty($row_surat_keluar->file_dokumen)) { ?>
			        					<a class="label label-success" href="<?php echo $__folder ?>app/surat_keluar_download.php?ref=<?php echo $ref ?>" target="_blank" title="Download"><?php echo $row_surat_keluar->file_dokumen; ?>
										</a>
			        				<?php } ?>
			                        <input type="hidden" id="file_dokumen2" name="file_dokumen2" value="<?php echo $row_surat_keluar->file_dokumen; ?>">  
								</div>
							</div>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowupd('frmsurat_keluar')==1) { ?>
                            <?php if($ref!='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Update" />
    						<?php } ?>
                        <?php } ?>
						
                        <?php if (allowadd('frmsurat_keluar')==1) { ?>
    						<?php if($ref=='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Simpan" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php if (allowdel('frmsurat_keluar')==1) { ?>
                            &nbsp;
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('surat_keluar_view') ?>'" />
