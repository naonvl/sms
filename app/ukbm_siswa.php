	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				if ($_POST['submit'] == 'Jawab') { $ref = ''; }
				//-----------------------------------------------/\
					
				include("app/exec/insert_ukbm_siswa.php");
				
				$tanggal = date("d-m-Y");
				if ($ref != "") {
					$sql=$select->list_ukbm_siswa($ref);
					$row_soal_siswa=$sql->fetch(PDO::FETCH_OBJ);	
					$idsiswa = $row_soal_siswa->idsiswa;
					$idsemester = $row_soal_siswa->idsemester;
					$tanggal = date("d-m-Y", strtotime($row_soal_siswa->tanggal));
					
					$ujian = "";
					if($row_soal_siswa->ujian == 1) {
						$ujian = "checked";
					}
				}		
			?>
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="soal_siswa" id="soal_siswa" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('tanggal,idsiswa,idsemester,idukbm');" >
            	...
            	<div class="form-group">
					...
					<div class="col-sm-4">
						<?php if($_SESSION["adm"] == 1 && $_SESSION["tipe_user"] != "Siswa") { ?>
							<select name="idsiswa" id="idsiswa" class="chosen-select form-control" >
								<option value=""></option>
								<?php select_siswa_id($idsiswa); ?>
							</select>
						<?php } else if($_SESSION["adm"] == 0 && $_SESSION["tipe_user"] == "Siswa") { ?>
							<select name="idsiswa1" id="idsiswa1" disabled="" class="chosen-select form-control" >
								<option value=""></option>
								<?php select_siswa_id($idsiswa); ?>
							</select>
				...
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">PAT *)</label>
					<div class="col-sm-5">
						<select name="idukbm" id="idukbm" class="chosen-select form-control" onchange="loadHTMLPost2('app/ukbm_siswa_ajax.php','ukbm_view','getukbm','idukbm')" >
							<option value=""></option>
							<?php 
				...
				<?php if($ref != "") { ?>
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Download File PAT</label>
						<div class="col-sm-5">
							<a class="label label-success" href="<?php echo $__folder ?>app/ukbm_download.php?ref=<?php echo $idukbm ?>" target="_blank" title="Download"><?php echo $row_soal_siswa->file_ukbm; ?>
							</a>
						</div>
					</div>
					
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Mengajukan Ujian PAT</label>
						<div class="col-sm-3">
							<input type="checkbox" id="ujian" name="ujian" class="ace" value="1" <?php echo $ujian ?> /><span class="lbl"></span>
						</div>
					</div>
				<?php } ?>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowadd('frmukbm_siswa')==1) { ?>
    						<?php if($ref=='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Save" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php if (allowupd('frmukbm_siswa')==1) { ?>
    						<?php if($ref!='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Update" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php if (allowdel('frmukbm_siswa')==1) { ?>
                            &nbsp;
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('ukbm_siswa_view') ?>'" />
