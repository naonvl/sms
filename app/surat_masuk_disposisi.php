	...
	function cekinput(fid) {  
	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='dari') {
			alert('Surat dari tidak boleh kosong!');				
		  }
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				$ref2 = notran(date('y-m-d'), 'frmsurat_masuk', '', '', ''); 
					
				include("app/exec/insert_surat_masuk_disposisi.php");
				
				$tanggal = date("d-m-Y");
				if ($ref != "") {
					$sql=$select->list_surat_masuk($ref);
					$row_surat_masuk=$sql->fetch(PDO::FETCH_OBJ);	
					
					$ref2		=	$row_surat_masuk->ref;
					$tanggal	=	date("d-m-Y", strtotime($row_surat_masuk->tanggal));
					
				}		
			?>
            
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="surat_masuk_ukbm" id="surat_masuk_ukbm" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('ref,tanggal,idkelompok_surat,petugas,tujuan');" >
            	...
            	<div class="row">
					<div class="col-sm-6">
						<div class="row">
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" id="no_surat" name="no_surat" class="form-control" value="<?php echo $row_surat_masuk->no_surat ?>"> 
								</div>
							</div>
			                ...
			                <div class="form-group">
								...
								<div class="col-sm-5">
									<select name="idkelompok_surat" id="idkelompok_surat" class="chosen-select form-control" >
										<option value=""></option>
										<?php populate_select("kelompok_surat","replid","nama",$row_surat_masuk->idkelompok_surat); ?>
									</select>
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" id="petugas" name="petugas" class="form-control" value="<?php echo $row_surat_masuk->petugas ?>"> 
								</div>
							</div>
					...
					<div class="col-sm-6">
						<div class="row">
							...
							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'Receipt From'; } else { echo 'Surat Dari'; } ?> *)</label>
								<div class="col-sm-5">
									<input type="text" id="dari" name="dari" class="form-control" value="<?php echo $row_surat_masuk->dari ?>"> 
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
			                        ...
			                        <br />
			        				<?php if (!empty($row_surat_masuk->file_dokumen)) { ?>
			        					<a class="label label-success" href="<?php echo $__folder ?>app/surat_masuk_download.php?ref=<?php echo $ref ?>" target="_blank" title="Download"><?php echo $row_surat_masuk->file_dokumen; ?>
										</a>
			        				<?php } ?>
			                        <input type="hidden" id="file_dokumen2" name="file_dokumen2" value="<?php echo $row_surat_masuk->file_dokumen; ?>">  
								</div>
							</div>
				...
				<?php
					include('surat_masuk_disposisi_detail.php');
				?>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('surat_masuk_disposisi_view') ?>'" />
