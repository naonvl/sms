...
<script>
	...
	function getpenerimaan() {
		...
		if(tnis != "") {
			document.location.href = "presensi_piket.php?nis="+tnis;
		} else {
			document.location.href = "presensi_piket.php";
		}
...
if ( $nis != "") 
{
	$sqlcek 	= "select replid from presensi_piket where date_format(tanggal,'%Y-%m-%d') = '$tanggal' and idpegawai='$idpegawai'";	
	...
	if($rowsph == 0) {
		...
		if($numrowssiswa > 0) {
			$sqlins		= "insert into presensi_piket (tanggal, idpegawai, hadir, ijin, sakit, alpa, keterangan, uid, dlu) values ('$tanggal', '$idpegawai', 1, 0, 0, 0, 'PC', '$uid', '$dlu')";
			$sql3=$dbpdo->prepare($sqlins);
			$sql3->execute();
			
			$sukses_absen = "ABSENSI PIKET BERHASIL";
		}
