...
<table style='font-size: 12px'>
  <tr>
      <th colspan='5' style='font-weight:bold; text-align: center;'>DAFTAR KETERLAMBATAN SISWA</th>
  </tr>
  <tr>
...
$x = 0;
$sqlx=$select->get_siswa_terlambat("", $tanggal, $tanggal1, $idsiswa, $idtingkat, $kelas, $all);
while($siswa_view=$sqlx->fetch(PDO::FETCH_OBJ)){
    ...
    $i = 0;
    $sql=$select->list_siswa_terlambat("", $tanggal, $tanggal1, $siswa_view->idsiswa, "", "", $all);
    while($siswa_terlambat_view=$sql->fetch(PDO::FETCH_OBJ)){
      ...
      $output .= '
        <tr>
          <td></td>
          <td>'.date("d-m-Y", strtotime($siswa_terlambat_view->tanggal)).'</td>
          <td>'.date("H:i", strtotime($siswa_terlambat_view->jam)).'</td>
          <td>'.$siswa_terlambat_view->alasan.'</td>
          <td>'.$siswa_terlambat_view->penanganan.'</td>
        </tr>
      ';
    }
}
