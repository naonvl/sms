...
<?php
        ...
        if (preg_match("/,/",$datacol) == 0) {
			...
			if($x > 0) {
				...
				$sakit		= numberreplace($column[($jumlah_ukbm1*2) + 5]);
				$izin		= numberreplace($column[($jumlah_ukbm1*2) + 6]);
				$alpa		= numberreplace($column[($jumlah_ukbm1*2) + 7]);
				$dispensasi	= numberreplace($column[($jumlah_ukbm1*2) + 8]);
				...
				if($nis != "") {
					...
					$nilai_sum 	= numberreplace($datanilai1->jumlah);
					...
					if($uas_p == "NULL") {
						...
						$nilai_sum 	= numberreplace($nilai_sum + $uas_p);
