...
<script type="text/javascript">
	function hapus(id) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('registrasireg_view') ?>/xm8r389xemx23xb2378e23/"+id+" ";
		}
	}
</script>
...
$nama		= $_REQUEST['nama'];
$verifikasi = $_REQUEST['verifikasi'];
$all		= $_REQUEST['all'];
...
<script type="text/javascript">
	...
	function verifikasi_registrasi(id) {
		if (confirm('Yakin verifikasi data ini?')) {
			document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('registrasireg_view') ?>/verif/"+id+" ";
		}
	}

	function unverifikasi_registrasi(id) {
		if (confirm('Yakin batalkan verifikasi data ini?')) {
			document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('registrasireg_view') ?>/unverif/"+id+" ";
		}
	}
	...
	<div class="row">
		<div class="col-xs-12">
			...
			if ($verif == "verif") 
			{
				...
				$update2=new update;
				$update2->update_registrasi_verif($segmen4);
			?>
				<script>
					alert("Verifikasi Data Sukses");
					window.location = '<?php echo $__folder ?><?php echo obraxabrix('registrasireg_view') ?>';
				</script>
			<?php
			}
			...
			if ($verif == "unverif") 
			{
				include 'class/class.update.php';
				$update2=new update;
				$update2->update_registrasi_unverif($segmen4);
			?>
				<script>
					alert("Batal Verifikasi Sukses");
					window.location = '<?php echo $__folder ?><?php echo obraxabrix('registrasireg_view') ?>';
				</script>
			<?php
			}
			...
			?>
                    ...
                    <meta http-equiv="Refresh" content="0;url=<?php echo $__folder . obraxabrix('registrasireg_view'); ?>" />
			<?php
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form class="form-horizontal" role="form" action="" method="post" name="siswa_lis" id="siswa_list" enctype="multipart/form-data" >
						...
						<div class="form-group">
							<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Verifikasi</label>
							<div class="col-sm-4">
								<select name="verifikasi" id="verifikasi" class="chosen-select form-control" >
									<option value=""></option>
									<?php select_verifikasi($verifikasi); ?>
								</select>
							</div>
						</div>
						...
						<table id="dynamic-table" class="table table-striped table-bordered table-hover" style="font-size: 12px">
							<thead>
								<tr>
									...
									<th style="font-weight:bold ">Bukti Transfer</th>
									<th style="font-weight:bold ">Verifikasi</th>
								</tr>
							</thead>

							<tbody>
                                <?php			
									$sql=$selectview->list_registrasi_verif("", "", $tahunmasuk, $idtingkat, $tanggal, $idjurusan, $nama, $all, $idproses, $idkelompok, $departemen, $verifikasi, $departemen_sekolah_id);	
									while ($registrasi_view=$sql->fetch(PDO::FETCH_OBJ)) {
										...
										$jenis_daftar = "";
										if($registrasi_view->idproses == 1) {
											$jenis_daftar = "Siswa Baru";
										}
										if($registrasi_view->idproses == 2) {
											$jenis_daftar = "Mutasi";
										} 
										//registrasireg_download
								?>
                                        ...
                                        <tr>
											...
											<td align="center">
												<?php 
												if($registrasi_view->verifikasi != 1) 
												{ 
													if (allowlvl('frmrregistrasireg')==1 || $_SESSION["adm"] == 1) 
													{
														echo "<a href='JavaScript:verifikasi_registrasi(\"$registrasi_view->replid\")' class='tooltip-error' data-rel='tooltip' title='Verifikasi'>
															<span class='red'>
												...
												{ 
													echo "<a href='JavaScript:unverifikasi_registrasi(\"$registrasi_view->replid\")' class='tooltip-error' data-rel='tooltip' title='Unverifikasi'><div style='background-color: green; color: #ffffff'>Verif</div></a>";
												} 
												?>
											</td>
										</tr>
