	...
	function cekinput(fid) {  
	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='idtingkat1') {
			alert('Tingkat tidak boleh kosong!');				
		  }

		  if (document.getElementById(arrf[i]).name=='jenis_absen') {
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				include("app/exec/insert_absensi_harian_siswa.php");
				...
				if ($ref != "") {
					$sql=$select->list_absensi_harian_siswa($ref);
					$absensi_harian_siswa_data=$sql->fetch(PDO::FETCH_OBJ);	
					
					$tanggal = date("d-m-Y", strtotime($absensi_harian_siswa_data->tanggal1));		
					$jam     = date("H", strtotime($absensi_harian_siswa_data->ts));
					$menit     = date("i", strtotime($absensi_harian_siswa_data->ts));	

					$jenis_absen = "";
					if($absensi_harian_siswa_data->ijin == 1) {
						$jenis_absen = "ijin";
					}
					if($absensi_harian_siswa_data->sakit == 1) {
						$jenis_absen = "sakit";
					}
					if($absensi_harian_siswa_data->alpa == 1) {
						$jenis_absen = "alpa";
					}
					if($absensi_harian_siswa_data->cuti == 1) {
						$jenis_absen = "cuti";
					}
				}		
			?>
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="absensi_harian_siswa2" id="absensi_harian_siswa2" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('tanggal,jam,idsiswa,idtingkat1,idkelas,jenis_absen');" >
				...
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Jam (HH:mm) *)</label>
					<div class="col-sm-3">
						<div class="input-group bootstrap-timepicker">
							<input id="jam" name="jam" type="text" class="form-control" value="<?= $jam.':'.$menit ?>" />
						</div>
					</div>
				</div>
                ...
                <div class="form-group">
					...
					<div class="col-sm-5">	
						<?php if($ref != "") { ?>					
							<select name="idsiswa" id="idsiswa" disabled class="chosen-select form-control" onchange="loadHTMLPost3('<?php echo $__folder ?>app/absensi_harian_siswa_ajax.php','tingkat_kelas_id','gettingkat_kelas','idsiswa')" >
								<option value=""></option>
								<?php select_siswa_kelas($absensi_harian_siswa_data->idsiswa) ?>
							</select>				
						<?php } else { ?>	
							<select name="idsiswa" id="idsiswa" class="chosen-select form-control" onchange="loadHTMLPost3('<?php echo $__folder ?>app/absensi_harian_siswa_ajax.php','tingkat_kelas_id','gettingkat_kelas','idsiswa')" >
								<option value=""></option>
								<?php select_siswa_kelas($absensi_harian_siswa_data->idsiswa) ?>
							</select>
						<?php } ?>
	                ...
	                <div class="form-group">
						...
						<div class="col-sm-3">
							<select name="idtingkat1" id="idtingkat1" disabled class="chosen-select form-control">
								<option value=""></option>
								<?php select_tingkat_unit("SMA", $absensi_harian_siswa_data->idtingkat); ?>
							</select>		
							<input type="hidden" name="idtingkat" id="idtingkat" value="<?= $absensi_harian_siswa_data->idtingkat ?>">		
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-3" id="kelas_id">
							<select name="idkelas1" id="idkelas1" disabled class="chosen-select form-control" >
								<option value=""></option>
								<?php select_kelas($absensi_harian_siswa_data->idtingkat, $absensi_harian_siswa_data->idkelas); ?>
							</select>		
							<input type="hidden" name="idkelas" id="idkelas" value="<?= $absensi_harian_siswa_data->idkelas ?>">	
						</div>
					</div>
				</div>
				
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Jenis Absen *)</label>
					<div class="col-sm-3">
						<select name="jenis_absen" id="jenis_absen" class="chosen-select form-control">
							<option value=""></option>
							<?php select_jenis_absen($jenis_absen); ?>
						</select>	
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-5">
						<textarea id="keterangan" name="keterangan" class="form-control"><?php echo $absensi_harian_siswa_data->keterangan ?></textarea>
					</div>
				</div>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowupd('rpt_absensi_harian_siswa')==1) { ?>
                            <?php if($ref!='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Update" />
    						<?php } ?>
                        <?php } ?>
						
                        <?php if (allowadd('rpt_absensi_harian_siswa')==1) { ?>
    						<?php if($ref=='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Simpan" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php if (allowdel('rpt_absensi_harian_siswa')==1) { ?>
                            &nbsp;
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('rpt_absensi_harian_siswa') ?>'" />
