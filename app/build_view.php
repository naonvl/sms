<script type="text/javascript">
	function hapus(id) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "<?php echo obraxabrix('build_view') ?>/xm8r389xemx23xb2378e23/"+id+" ";
		}
	}
</script>
...
<div class="page-content">			
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<form class="form-horizontal" role="form" action="" method="post" name="build_view" id="build_view" class="form-horizontal" enctype="multipart/form-data" >
				...
				<div class="form-group">
                    ...
                    <div class="col-sm-3">
                    	 <select id="asset_type_id" name="asset_type_id" class="chosen-select form-control"  style="max-width: 300px; font-size: 12px;" >
                          	<option value=""></option>
                            <?php 
                                combo_select_active("asset_type","id","type","active","1",$asset_type_id) 
                            ?>	
            	...
            	<div class="form-group">
					...
					<div class="col-sm-5">
						<input type="text" id="asset_name" name="asset_name" value="<?php echo $asset_name ?>">								
					</div>
				</div>
		...
		<div class="col-xs-12">
            ...
            <?php
                ...
                if ($delete == "xm8r389xemx23xb2378e23") {
					...
					$delete2=new delete;
					$delete2->delete_build($segmen4);
			?>
					<div class="alert alert-success">
						<strong>Delete Data successfully</strong>
					</div>
                    
                    <meta http-equiv="Refresh" content="0;url=<?php echo $nama_folder . '/' . obraxabrix(build_view); ?>" />
			<?php
					...
					<div>
						<table id="dynamic-table" class="table table-striped table-bordered table-hover">
							...
							<tbody>
                                <?php			
									$i = 0;
            						$sql=$select->list_build($ref, $build_name, $alias, $tanggal_perolehan, $group_block, $asset_type_id, $all);
						            while($row_build=$sql->fetch(PDO::FETCH_OBJ)){
								...
								?>
                                        ...
                                        <tr>
                                            <td><?php echo $i ?></td>
                                            <td><?php echo $row_build->ref ?></td>
					                    	<td><?php echo $row_build->ref_id ?></td>
					                    	<td><?php echo $row_build->asset_name ?></td>
				                            <td><?php echo $row_build->asset_type ?></td>
					                    	<td><?php echo $row_build->alamat ?></td>
					                    	<td>
												<?php if ($row_build->active == 1) { ?>
											...
											<td>
                                                ...
                                                <?php if (allowupd('frmbuild')==1) { ?>
    												<a href="<?php echo obraxabrix('build') ?>/<?php echo $row_build->ref ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
    													<span class="green">
                                                ...
                                                <?php if (allowdel('frmbuild')==1) { ?>    
                                                    &nbsp;
    												<a href="JavaScript:hapus('<?php echo $row_build->ref ?>')" class="tooltip-error" data-rel="tooltip" title="Delete">
    													<span class="red">
