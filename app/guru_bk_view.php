...
<script type="text/javascript">
	function hapus(id) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('guru_bk_view') ?>/xm8r389xemx23xb2378e23/"+id+" ";
		}
	}
</script>
		...
		<div class="col-xs-12">
            ...
            <?php
				...
				if ($delete == "xm8r389xemx23xb2378e23") {
					...
					$delete2=new delete;
					$delete2->delete_guru_bk($segmen4);
			?>
					<div class="alert alert-success">
						<strong>Delete Data successfully</strong>
					</div>
                    
                    <meta http-equiv="Refresh" content="0;url=<?php echo $__folder ?><?php echo obraxabrix('guru_bk_view') ?>" />
			<?php
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form class="form-horizontal" role="form" action="" method="post" name="guru_bk_view" id="guru_bk_view" enctype="multipart/form-data" >
		            	...
		            	<div class="form-group">
							...
							<div class="col-sm-3">
								<select name="idangkatan" id="idangkatan" class="chosen-select form-control" >
									<option value=""></option>
									<?php select_thnajaran($guru_bk_data->idtahunajaran); ?>
								</select>
							</div>
						</div>
					...
					<div>
						<table id="dynamic-table" class="table table-striped table-bordered table-hover">
							...
							<tbody>
                                <?php			
									$i = 0;
            						$sql=$select->list_guru_bk("", $idtahunajaran, $idguru, $aktif, $all, $departemen);
						            while($guru_bk_view=$sql->fetch(PDO::FETCH_OBJ)){
								...
								?>
                                        ...
                                        <tr>
                                            <td><?php echo $i ?></td>
											<td><?php echo $guru_bk_view->departemen ?></td>
											<td><?php echo $guru_bk_view->tahunajaran ?></td>
											<td><?php echo $guru_bk_view->nip . ' ' .$guru_bk_view->nama_guru ?></td>
											<td>
												<?php if ($guru_bk_view->aktif == 1) { ?>
											...
											<td align="center">
                                                ...
                                                <?php if (allowupd('frmguru_bk')==1) { ?>
    												<a href="<?php echo $__folder ?><?php echo obraxabrix('guru_bk') ?>/<?php echo $guru_bk_view->replid ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
    													<span class="green">
                                                ...
                                                <?php if (allowdel('frmguru_bk')==1) { ?>    
                                                    &nbsp;
    												<a href="JavaScript:hapus('<?php echo $guru_bk_view->replid ?>')" class="tooltip-error" data-rel="tooltip" title="Delete">
    													<span class="red">
