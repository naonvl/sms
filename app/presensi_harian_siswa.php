...
function kirim_wa($nisn, $jam, $namasekolah, $sukses_absen) {
	...
	if($sukses_absen == "TERLAMBAT") {
		$keterangan = "*Terlambat* di sekolah";
	}
...
if ( $nis != "") 
{
	...
	$sqlstr	= "select replid, jam_masuk, jam_pulang_cepat, jam_pulang from presensiharian_setup where departemen='$departemen' and departemen_sekolah_id='$departemen_sekolah_id' order by replid desc limit 1 ";
	...
	$keterangan = "";
	if($rowssetup > 0) {
		$datasetup=$sqlx->fetch(PDO::FETCH_OBJ);
		...
		/*echo 'masuk:'.date('H:i', strtotime($datasetup->jam_masuk)).'<br>';
		echo 'cepat:'.date('H:i', strtotime($datasetup->jam_pulang_cepat)).'<br>';
		echo 'pulang:'.date('H:i', strtotime($datasetup->jam_pulang)).'<br>';*/

		if( ($time < $time_out) && ($time > $time_in) ) {
			if($rowsphs == 1) {
				$keterangan = "PULANG CEPAT";
			}
		}
		...
		if($keterangan == "") {
			if($rowsphs == 1 && $time < $time_out) {
				$keterangan = "SISWA SUDAH PERNAH ABSEN";
			}
		}
	}
	//-----------end check---------
	...
	if($rowsphs < 2) {
		...
		if($numrowssiswa > 0 && $idkelas != "") {
			if($rowsphs == 0) {
				$sqlphs		= "insert into phsiswa (idpresensi, nis, hadir, keterangan, ts, token, issync, departemen, departemen_sekolah_id, info1, info3) values ('$idpresensi', '$nis', 1, 'Kartu', '$dlu', 0, 0, '$departemen', '$departemen_sekolah_id', 'datang', '$keterangan')";
				$sql7=$dbpdo->prepare($sqlphs);
				$sql7->execute();
			} else {
				if($rowsphs == 1 && ($keterangan == "PULANG CEPAT") ) {
					$sqlphs		= "insert into phsiswa (idpresensi, nis, hadir, keterangan, ts, token, issync, departemen, departemen_sekolah_id, info1, info3) values ('$idpresensi', '$nis', 1, 'Kartu', '$dlu', 0, 0, '$departemen', '$departemen_sekolah_id', 'pulang', '$keterangan')";
					$sql7=$dbpdo->prepare($sqlphs);
					$sql7->execute();
				}

				if( $time >= $time_out && $rowsphs == 1 ) {
					if($keterangan == "") {
						$sqlphs		= "insert into phsiswa (idpresensi, nis, hadir, keterangan, ts, token, issync, departemen, departemen_sekolah_id, info1, info3) values ('$idpresensi', '$nis', 1, 'Kartu', '$dlu', 0, 0, '$departemen', '$departemen_sekolah_id', 'pulang', '$keterangan')";
						$sql7=$dbpdo->prepare($sqlphs);
						$sql7->execute();

						$keterangan = "ABSENSI PULANG BERHASIL";
					}
				}
