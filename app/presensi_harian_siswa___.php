...
if ( $nis != "") 
{
	...
	if($rowsphs < 2) {
		...
		if($numrowssiswa > 0 && $idkelas != "") {
			if($rowsphs == 0) {
				$sqlphs		= "insert into phsiswa (idpresensi, nis, hadir, keterangan, ts, token, issync, departemen, departemen_sekolah_id, info1) values ('$idpresensi', '$nis', 1, 'Kartu', '$dlu', 0, 0, '$departemen', '$departemen_sekolah_id', 'datang')";
				$sql7=$dbpdo->prepare($sqlphs);
				$sql7->execute();
			} else {
				if($rowsphs == 1 && ($keterangan == "PULANG CEPAT") ) {
					$sqlphs		= "insert into phsiswa (idpresensi, nis, hadir, keterangan, ts, token, issync, departemen, departemen_sekolah_id, info1) values ('$idpresensi', '$nis', 1, 'Kartu', '$dlu', 0, 0, '$departemen', '$departemen_sekolah_id', 'pulang')";
					$sql7=$dbpdo->prepare($sqlphs);
					$sql7->execute();
				}
