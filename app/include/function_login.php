	...
	if($username && $password) {
	  ...
	  if (!empty($data_cek->pwd)) {	  	  		
		  ...
		  if($rr->usrid != ""){
				...
				$sqlstr="select replid from departemen where departemen='$departemen'";
				$rsltepy=$dbpdo->query($sqlstr);
				$dataepy=$rsltepy->fetch(PDO::FETCH_OBJ);		
				$departemen_id = $dataepy->replid;

				//a.departemen_id='$departemen_id' and 
				$sqlstr="select a.departemen_id, a.departemen_sekolah_id, b.nama nama_sekolah from usr_departemen a left join departemen_sekolah b on a.departemen_sekolah_id=b.replid where usrid='$username' limit 1";
				$rsdep=$dbpdo->query($sqlstr);
				$datadep=$rsdep->fetch(PDO::FETCH_OBJ);	
				$_SESSION["departemen_id"] 			= $datadep->departemen_id;
				$_SESSION["departemen_sekolah_id"] 	= $datadep->departemen_sekolah_id;
				$_SESSION["nama_sekolah"] 			= $datadep->nama_sekolah;
