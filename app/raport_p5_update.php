...
<script type="text/javascript">
	...
	function checkvalidasi() {
		var itahunajaran_id = document.getElementById('tahunajaran_id').value;
		if(tahunajaran_id == "") {
		...
		if(guru_id == "") {
			alert("Guru Pembimbing tidak boleh kosong!!!");
			return false
		}
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<?php
					...
					if ($ref != "") {
						$sql=$select->list_raport_p5_view("", $tahunajaran_id, $tingkat_id, $semester_id, $guru_id, $all, '', $ref);
						$row_report_p5	=	$sql->fetch(PDO::FETCH_OBJ);	
						$replid					= 	$row_report_p5->replid;
						$ref2 					= 	$row_report_p5->syscode;	
						$departemen 		=		$row_report_p5->departemen;
						$tahunajaran_id =		$row_report_p5->idtahunajaran;
						$tingkat_id 		=		$row_report_p5->idtingkat;
						$idkelas 				=		$row_report_p5->idkelas;
						$semester_id 		=		$row_report_p5->idsemester;
						$guru_id 				=		$row_report_p5->idguru;
						$tema_id 				=		$row_report_p5->tema_id;
						$tema 					=		$row_report_p5->tema;
						$deskripsi_p5_id=		$row_report_p5->deskripsi_p5_id;

						##get multi siswa
						$idsiswa 				=		"";
						$sqlidsiswa = $select->list_siswa_raport_p5('', $tingkat_id, $idkelas, $tahunajaran_id, $semester_id, '', '', '', $ref);
						while($datasiswa = $sqlidsiswa->fetch(PDO::FETCH_OBJ)) {
								if($idsiswa == "") {
						...
						<div class="form-group">
							...
							<div class="col-sm-3">
								<select name="idtingkat" id="idtingkat" class="chosen-select form-control" onchange="loadHTMLPost2('app/daftar_nilai_ajax.php','kelas_id','getkelas','idtingkat')" >
									<option value=""></option>
									<?php select_tingkat_unit("SMA", $tingkat_id); ?>
								</select>								
							</div>
						</div>
				...
				<div class="row">
                    ...
                    <?php select_siswa_multi($idsiswa, $idkelas); ?>
						...
						<div class="form-group">
							...
							<div class="col-sm-4">						
								<select name="tema_id" id="tema_id" class="chosen-select form-control" >
									<option value=""></option>
									<?php select_raport_p5_tema($tema_id) ?>
								</select>						
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-4">						
								<textarea id="tema" name="tema" class="form-control" autocomplete="off" rows="5"><?= $tema ?></textarea>					
							</div>
						</div>
	                  ...
	                  <th class="center" style="font-weight:bold ">No.</th>
								...
								<tbody>
									<?php			
                    $i = 0;
        						$sql=$select->get_deskripsi_raport_p5("", $tahunajaran_id, '', '', '', '');
				            while($deskripsi_raport_p5_view=$sql->fetch(PDO::FETCH_OBJ)){
	            							
        							$pilih 	=	"";
											if($deskripsi_p5_id == $deskripsi_raport_p5_view->replid) {
												$pilih = "checked";
											}
									?>
                        		...
                        		<tr style="background: #d3fe7a">
                        			...
                        			<td align="center">Dimensi</td>
                          			...
                          			while($row_det=$sql2->fetch(PDO::FETCH_OBJ)) {	
                        						...
                        						$sqlstr = "select distinct dimensi_id from raport_p5_dimensi where syscode='$ref2' and dimensi_id='$row_det->replid'";
                        						$sqldms=$dbpdo->prepare($sqlstr);
																		$sqldms->execute();
																		$datadms = $sqldms->fetch(PDO::FETCH_OBJ);
																		$dimensi_id = $datadms->dimensi_id;

																		$pilih  = "";
																		if($dimensi_id == $row_det->replid) {
																				$pilih = "checked";
																		}
                        			?>
                          				<tr style="font-weight: bold;">
	                            			...
	                            			<td align="center">
						                        	<input type="checkbox" name="pilih_<?= $i.$no ?>" id="pilih_<?= $i.$no ?>" value="1" class="ace input-lg" <?= $pilih ?> ><span class="lbl bigger-120"></span>
						                        </td>
	                            		</tr>
	                            				...
	                            				<table width="100%" border="1" style="border: 1px solid #93d145;">
		                            				<?php
		                            					...
		                            					while($datasub = $sqlsub->fetch(PDO::FETCH_OBJ)) {
		                            						...
		                            						$sqlstr = "select distinct elemen_id from raport_p5_elemen where syscode='$ref2' and dimensi_id='$row_det->replid' and elemen_id='$datasub->replid'";
		                            						$sqlel=$dbpdo->prepare($sqlstr);
																						$sqlel->execute();
																						$datael = $sqlel->fetch(PDO::FETCH_OBJ);
																						$elemen_id = $datael->elemen_id;

																						$select_elemen  = "";
																						if($elemen_id == $datasub->replid) {
																							$select_elemen = "checked";
																						}
		                            				?>
		                            						...
		                            						<tr style="background: #465fbe; color: white; font-weight: bold;">
						                            			...
						                            			<td align="center">
						                            				<input type="checkbox" class="ace input-lg" name="select_elemen_<?= $i.$no.$x ?>" id="select_elemen_<?= $i.$no.$x ?>" value="1" <?= $select_elemen ?> ><span class="lbl bigger-120"></span>
							                            		</td>
																						</tr>
				                            					...
				                            					while($datasubelemen = $sqlsubelemen->fetch(PDO::FETCH_OBJ)) {
				                            							...
				                            							$sqlstr = "select distinct subelemen_id from raport_p5_nilai where syscode='$ref2' and subelemen_id='$datasubelemen->replid' and elemen_id='$datasub->replid'";
					                            						$sqlsubel=$dbpdo->prepare($sqlstr);
																									$sqlsubel->execute();
																									$datasubel = $sqlsubel->fetch(PDO::FETCH_OBJ);
																									$subelemen_id = $datasubel->subelemen_id;

																									$select_subelemen  = "";
																									if($subelemen_id == $datasubelemen->replid) {
																										$select_subelemen = "checked";
																									}
				                            				?>
	                            		...
	                            		<?php /* <tr>
	                            			...
	                            			<td>
	                            				<table width="100%" border="1" style="border: 1px solid #93d145;">
		                            				<?php
		                            					$x=0;
		                            					$sqlsub = $select->get_deskripsi_raport_p5_nilai($row_det->replid);
		                            					while($datasub = $sqlsub->fetch(PDO::FETCH_OBJ)) {
		                            				?>
		                            						<tr style="background: #eff5f8">
						                            			<td align="center" width="2%">:></td>
						                            			<td align="left"><?= $datasub->deskripsi ?></td>
																						</tr>	
		                            				<?php
		                            						$x++;
		                            					}
                        			...
                        			?>
												...
												<td align="center">
                        	<input type="radio" name="pilih[]" id="pilih_<?= $i ?>" value="<?= $deskripsi_raport_p5_view->replid ?>" class="ace" <?= $pilih ?> ><span class="lbl"></span>
                        </td>
		            ...
		            <?php if (allowadd('raport_p5')==1) { ?>
		    						<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Update" onClick="return confirm('Yakin data sudah benar?')" />
                <?php } ?>
