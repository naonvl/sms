 <table class="table table-bordered table-condensed table-hover table-striped" style="width: auto; font-size: 12px">
	<thead>
		<tr style="color: #168124; font-weight: bold; border: 1px solid #ccc; background-color: #e6ffea"> 
			...
			<?php
				$no = 0;
				$sqlhari = $select->list_jam("", $hari, "", $departemen);
	...
	<tbody>
		...
		<?php
			$no = 0;
			$jmldata = 0;
			$sql=$select->list_kelas("", $idtingkat, "", $departemen);
				...
				<tr style="background-color:ffffff;" > 
					...
					?>
							<td align="center" style="font-size: 14px; font-weight: bold;">
								...
								?>
									<a href="javascript:void(0);" name="Find" title="Input Jadwal" onClick="jadwal_input('<?php echo $hari ?>','<?php echo $idtingkat ?>','<?php echo $row_kelas_detail->replid ?>','<?php echo $idguru ?>','<?php echo $data_hari->replid ?>','<?= $departemen ?>')"><img src="assets/img/plus.png" /></a>
								<?php 
										}
									}  else {
										if($data_hari->istirahat == 0) {
									?>
											<a href="javascript:void(0);" name="Find" title="Input Jadwal" onClick="jadwal_update('<?php echo $hari ?>','<?php echo $idtingkat ?>','<?php echo $row_kelas_detail->replid ?>','<?php echo $idguru ?>','<?php echo $data_hari->replid ?>','<?php echo $data_jdl->replid ?>','<?= $data_jdl->departemen ?>')" style="color: #ff0000" >
									<?php
