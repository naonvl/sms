...
class protection{
	...
	function protection_departemen_sekolah($ref=''){
		
		$dbpdo = DB::create();
		
		$sqlstr		=	"select id from usr_departemen where departemen_sekolah_id='$ref' limit 1";
		$sql		=	$dbpdo->prepare($sqlstr);
		$sql->execute();
		$rows 		= 	$sql->rowCount();

		if($rows == 0) {
			$sqlstr		=	"select replid from calonsiswa where departemen_sekolah_id='$ref' limit 1";
			$sql		=	$dbpdo->prepare($sqlstr);
			$sql->execute();
			$rows 		= 	$sql->rowCount();
		}
