...
class update{
	...
	function update_usr($ref,$photo){
		...
		try {
			...
			for ($i=1; $i<=$usr_jmldata3; $i++) {
				...
				$old_slc_unit 	= (empty($_POST['old_slc_unit_'.$i.''])) ? 0 : $_POST['old_slc_unit_'.$i.''];
				$departemen_id 	= $_POST['departemen_id_'.$i.''];
				$departemen_sekolah_id = $_POST['departemen_sekolah_id_'.$i.''];
				$old_id		= $_POST['old_id_'.$i.''];
								
				if ($old_slc_unit==1) {
					if ($slc_unit==1) {
						$sqlstr = "update usr_departemen set usrid='$usrid', departemen_id=$departemen_id, departemen_sekolah_id='$departemen_sekolah_id' where usrid='$old_usrid' and id='$old_id' ";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
					} else {
						$sqlstr = "delete from usr_departemen where usrid='$old_usrid' and id='$old_id'";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
					}	
				} 
	...
	//------update registrasi upload bukti transfer
	function update_registrasi_upload($token){
		...
		try {
            ...
            //-----------upload file photo
			$uploaddir 		= '../app/file_transfer_registrasi/';
			$info32	= $_POST["info32"];
			...
			$sqlstr = "update calonsiswa set info3='$info3' where token='$token' ";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	//------update registrasi upload berkas
	function update_registrasi_upload_berkas($token){
		...
		try {
			...
			if($file_raport != "") {	
				...
				if($file_raport != $file_raport2) {
					...
					$file_raport = $token . '_' . $file_raport;
				}
			...
			if($file_memo_ortu != "") {	
				...
				if($file_memo_ortu != $file_memo_ortu2) {
					...
					$file_memo_ortu = $token . '_' . $file_memo_ortu;
				}
			...
			//-----------upload file kk
			$uploaddir 		= '../app/file_ktp/';
			$file_ktp2	= $_POST["file_ktp2"];
			$file_ktp 	= $_FILES['file_ktp']['name']; 
			$tmpname  		= $_FILES['file_ktp']['tmp_name'];
			$filesize 		= $_FILES['file_ktp']['size'];
			$filetype 		= $_FILES['file_ktp']['type'];
			...
			if($file_ktp != "") {	
				...
				if($file_ktp != $file_ktp2) {
				
					if(!empty($file_ktp2)) {
						unlink($uploaddir . $file_ktp2); //remove file 
					}
					
					$file_ktp = $token . '_' . $file_ktp;
				}
							
				$uploadfile = $uploaddir . $file_ktp;		
				if (move_uploaded_file($_FILES['file_ktp']['tmp_name'], $uploadfile)) {
					echo "";											
				} 
			}
			//----------------	
			...
			$sqlstr = "update calonsiswa set file_raport='$file_raport', file_memo_ortu='$file_memo_ortu', file_ktp='$file_ktp', file_akte='$file_akte', file_kk='$file_kk' where token='$token' ";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function update_registrasi($ref){
		...
		try {
			...
			for ($x=0; $x<=$jmldata; $x++) {
				...
				$nama_			 	=	petikreplace($_POST['nama_'.$x.'']);
			...
			for ($x=0; $x<=$jmldata2; $x++) {
				...
				$penyelenggara2_	=	petikreplace($_POST['penyelenggara2_'.$x.'']);
	...
	function update_departemen($ref){
		...
		try {
			...
			for($i=0; $i<=$jmldata; $i++) {
				...
				if($delete == 0) {
					$sqlstr = "select replid from departemen_sekolah where departemen_id='$ref' and replid='$old_id'";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
					$rows = $sql->rowCount();
					...
					if($rows > 0) {
						if($link_registrasi != "") {
							$link_registrasi__ = explode("?", $link_registrasi);
							$link_registrasi =  $link_registrasi__[0]."?unit=".$old_id;
						} else {
							$link_registrasi = $link.$old_id;
						}
						

						$sqlstr = "update departemen_sekolah set departemen_id='$ref', nama='$nama', alamat='$alamat', nipkepsek='$nipkepsek', kuota='$kuota', tanggal_awal='$tanggal_awal', tanggal_akhir='$tanggal_akhir', link_registrasi='$link_registrasi', biaya='$biaya', bank='$bank', no_rekening='$no_rekening', nama_pemilik='$nama_pemilik' where departemen_id='$ref' and replid='$old_id'";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
					} else {
						if($nama != "") {
							$sqlstr = "insert into departemen_sekolah (departemen_id, nama, alamat, nipkepsek, kuota, tanggal_awal, tanggal_akhir, link_registrasi, biaya, bank, no_rekening, nama_pemilik) values ('$ref', '$nama', '$alamat', '$nipkepsek', '$kuota', '$tanggal_awal', '$tanggal_akhir', '$link_registrasi', '$biaya', '$bank', '$no_rekening', '$nama_pemilik')";
							$sql=$dbpdo->prepare($sqlstr);
							$sql->execute();
							...
							$sqlstr = "update departemen_sekolah set link_registrasi='$link_registrasi' where departemen_id='$ref' and replid='$sekolah_id'";
							$sql=$dbpdo->prepare($sqlstr);
							$sql->execute();
						}
					}
				} else {
					$sqlstr = "delete from departemen_sekolah where departemen_id='$ref' and replid='$old_id'";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
				}
	...
	function update_tingkat($ref){
		...
		try {
			...
			$sqlstr = "update tingkat set tingkat='$tingkat', departemen='$departemen', departemen_sekolah_id='$departemen_sekolah_id', aktif='$aktif', keterangan='$keterangan', urutan='$urutan', tanggal_ttd='$tanggal_ttd', ts='$dlu' where replid='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();		
	...
	function update_kelas($ref){
		...
		try {
			...
			$sqlstr = "update kelas set idtahunajaran='$idtahunajaran', departemen_sekolah_id='$departemen_sekolah_id', idtingkat='$idtingkat', kelas='$kelas', kapasitas='$kapasitas', nipwali='$nipwali', aktif='$aktif', keterangan='$keterangan', ttd_file_hd='$ttd_file_hd_trf', ts='$dlu' where replid='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();	
	...
	function update_tahunajaran($ref){
		...
		try {
			...
			$sqlstr = "update tahunajaran set departemen='$departemen', tahunajaran='$tahunajaran', tglmulai='$tglmulai', tglakhir='$tglakhir', aktif='$aktif', info1='$info1', info2='$info2', info3='$ttd_file_trf', keterangan='$keterangan', ts='$dlu' where replid='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();		

			$sqlstr = "update tahunajaran set info2='0' where replid<>'$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function update_pegawai($ref){
		...
		try {
			...
			$sqlstr = "update pegawai set bagian='$bagian', nip='$nip', nrk='$nrk', nama='$nama', panggilan='$panggilan', kelamin='$kelamin', gelar='$gelar', gelar_belakang='$gelar_belakang', tmplahir='$tmplahir', tgllahir='$tgllahir', agama='$agama', suku='$suku', jabatan_id='$jabatan_id', pangkat_id='$pangkat_id', nikah='$nikah', jenis_id='$jenis_id', pendidikan_terakhir_id='$pendidikan_terakhir_id', jurusan='$jurusan', tahun_lulus='$tahun_lulus', universitas='$universitas', noid='$noid', alamat='$alamat', telpon='$telpon', handphone='$handphone', email='$email', karpeg='$karpeg', no_sertifikasi='$no_sertifikasi', idjenis_sertifikasi='$idjenis_sertifikasi', npwp='$npwp',	nuptk='$nuptk', tmt_cpns='$tmt_cpns', unit_cpns='$unit_cpns', no_sk_masuk='$no_sk_masuk', idstatus_pegawai='$idstatus_pegawai', nik='$nik', nama_ibu='$nama_ibu', nama_pasangan='$nama_pasangan', tempat_lahir_pasangan='$tempat_lahir_pasangan', tanggal_lahir_pasangan='$tanggal_lahir_pasangan', tanggal_nikah='$tanggal_nikah', tempat_nikah='$tempat_nikah', pekerjaan_pasangan='$pekerjaan_pasangan', instansi_pasangan='$instansi_pasangan', nip_pasangan='$nip_pasangan', keluarga_tanggungan='$keluarga_tanggungan', usia='$usia', ajar_lain='$ajar_lain', jumlah_jam_induk='$jumlah_jam_induk', jumlah_jam_ajar_lain='$jumlah_jam_ajar_lain', nama_bank='$nama_bank', unit_bank='$unit_bank', no_rek='$no_rek', nama_pemilik='$nama_pemilik', desa='$desa', kecamatan='$kecamatan', kode_pos='$kode_pos', tanggal_pensiun='$tanggal_pensiun', foto_file='$foto_file', keterangan='$keterangan', no_sk_tetap='$no_sk_tetap', tanggal_sk_tetap='$tanggal_sk_tetap', ts='$ts' where replid='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();		
	...
	//------update pegawai non pns
	function update_pegawai_non_pns($ref){
		...
		try {
			...
			$sqlstr = "update pegawai set bagian='$bagian', jabatan_id='$jabatan_id', nip='$nip', nama='$nama', panggilan='$panggilan', kelamin='$kelamin', gelar='$gelar', gelar_belakang='$gelar_belakang', tmplahir='$tmplahir', tgllahir='$tgllahir', agama='$agama', suku='$suku', nikah='$nikah', jenis_id='$jenis_id', noid='$noid', alamat='$alamat', telpon='$telpon', handphone='$handphone', email='$email', karpeg='$karpeg', no_sertifikasi='$no_sertifikasi', idjenis_sertifikasi='$idjenis_sertifikasi', pendidikan_terakhir_id='$pendidikan_terakhir_id', jurusan='$jurusan', tahun_lulus='$tahun_lulus', universitas='$universitas', npwp='$npwp', nuptk='$nuptk', tmt_cpns='$tmt_cpns', unit_cpns='$unit_cpns', no_sk_masuk='$no_sk_masuk', no_skkki='$no_skkki', tahun_skkki='$tahun_skkki', idstatus_pegawai='$idstatus_pegawai', nik='$nik', nama_ibu='$nama_ibu', nama_pasangan='$nama_pasangan', tempat_lahir_pasangan='$tempat_lahir_pasangan', tanggal_lahir_pasangan='$tanggal_lahir_pasangan', tanggal_nikah='$tanggal_nikah', tempat_nikah='$tempat_nikah', pekerjaan_pasangan='$pekerjaan_pasangan', instansi_pasangan='$instansi_pasangan', nip_pasangan='$nip_pasangan', keluarga_tanggungan='$keluarga_tanggungan', usia='$usia', ajar_lain='$ajar_lain', jumlah_jam_induk='$jumlah_jam_induk', jumlah_jam_ajar_lain='$jumlah_jam_ajar_lain', nama_bank='$nama_bank', unit_bank='$unit_bank', no_rek='$no_rek', nama_pemilik='$nama_pemilik', desa='$desa', kecamatan='$kecamatan', kode_pos='$kode_pos', tanggal_pensiun='$tanggal_pensiun', foto_file='$foto_file', keterangan='$keterangan', no_sk_tetap='$no_sk_tetap', tanggal_sk_tetap='$tanggal_sk_tetap', ts='$ts' where replid='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();		
	...
	function update_pelajaran($ref){
		...
		try {
			...
			$sqlstr = "update pelajaran set kode='$kode', nama='$nama', departemen='$departemen', departemen_sekolah_id='$departemen_sekolah_id', sifat='$sifat', ekstra_sifat='$ekstra_sifat', pegawai_id='$pegawai_id', aktif='$aktif', keterangan='$keterangan', info1='$sks', info2='$kkm_p', info3='$uid', alias='$alias', idpelajaran_alias='$idpelajaran_alias', minat='$minat', bentrok='$bentrok', merdeka='$merdeka', token='$kkm_k', ts='$dlu' where replid='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();		
	...
	function update_dasarpenilaian($ref){
		...
		try {
			...
			$sqlstr = "update dasarpenilaian set departemen='$departemen', dasarpenilaian='$dasarpenilaian', keterangan='$keterangan', ts='$dlu' where replid='$ref'";
	...
	function update_kompetensi($ref){
		...
		try {
			...
			$sqlstr = "update kompetensi set departemen='$departemen', kode='$kode', kompetensi='$kompetensi', aktif='$aktif', dlu='$dlu' where replid='$ref'";
		...
		try {
			...
			for($y=0; $y<$jmldata_jenis_aspek; $y++) {
				...
				for($z=0; $z<$jml_aspek_detail; $z++) {
					...
					$old_line			=	numberreplace($_POST['old_line_'.$y.$z.'']);
	...
	function update_kartu_rencana_studi($ref){
		...
		try {
			...
			$sqlstr = "update kartu_rencana_studi set departemen='$departemen', peminatan='$peminatan', tingkat_id='$tingkat_id', kelompok_pelajaran_id='$kelompok_pelajaran_id', pelajaran_kode='$pelajaran_kode', pelajaran_id='$pelajaran_id', semester_id='$semester_id', sks='$sks', idtahunajaran='$idtahunajaran', urutan='$urutan', kode1='$kode1', kode2='$kode2', uid='$uid', dlu='$dlu' where replid='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();		
	...
	function update_pemetaan_kd($ref){
		...
		try {
			...
			//----------------
			$sqlstr="update pemetaan_kd set departemen='$departemen', permen_kd_id='$permen_kd_id', idtingkat='$idtingkat', idpelajaran='$idpelajaran', kd='$kd', kode='$kode', uraian='$uraian', kkm_sekolah='$kkm_sekolah', kkm_pelajaran='$kkm_pelajaran', jumlah_ukbm='$jumlah_ukbm', urutan='$urutan', aktif='$aktif', uid='$uid', dlu='$dlu' where replid='$ref' ";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
		}
	...
	function update_pemetaan_kd_siswa($ref){
		...
		try {
			...
			//----------------
			$sqlstr="update pemetaan_kd_siswa set departemen='$departemen', pemetaan_kd_id='$pemetaan_kd_id', idsiswa='$idsiswa', idguru='$idguru', idpelajaran='$idpelajaran', nilai='$nilai', uid='$uid', dlu='$dlu' where replid='$ref' ";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
		}
	...
	function update_buku_kunjungan($ref){
		...
		try {
			...
			$sqlstr = "update buku_kunjungan set tanggal='$tanggal', nama='$nama', instansi='$instansi', telepon='$telepon', ttd='$ttd', keperluan='$keperluan', kesan_pesan='$kesan_pesan', keterangan='$keterangan', uid='$uid', dlu='$dlu' where ref='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
		}
	...
	function update_room_booking($ref){
		...
		try {
			...
			$registration_s = $_POST["registration_slc"];
			$exp_registration_s = explode("|", $registration_s);
			
			$old_id = $_POST["old_id"];
			$exp_old_id = explode("|", $old_id);
			
			$delete_id = $_POST["delete_id"];
			$exp_delete_id = explode("|", $delete_id);

			$jmldata = count($exp_room_ref);		
			for ($i=0; $i<=$jmldata; $i++) {
				$registration_slc = $exp_registration_s[$i]; 
				if($registration_slc == true) {
					$registration_slc = 1;
				}
				if($registration_slc == false) {
					$registration_slc = 0;
				}
				
				$delete 		  = $exp_delete_id[$i]; 
				if($delete == true) {
					$delete = 1;
				}
				if($delete == false) {
				...
				$old 	 		  = $exp_old_id[$i]; //(empty($_POST[old_.$i])) ? 0 : $_POST[old_.$i];
				
				$room_ref 		  = $exp_room_ref[$i]; //$_POST[room_ref_.$i];
	...
	function update_siswa_terlambat($ref){
		...
		try {
			...
			$sqlstr = "update siswa_terlambat set tanggal='$tanggal', jam='$jam', idsiswa='$idsiswa', idtingkat='$idtingkat', idkelas='$idkelas', alasan='$alasan', penanganan='$penanganan', uid='$uid', dlu='$dlu' where replid='$ref'";
	...
	function update_kejadian_lain($ref){
		...
		try {
			...
			$sqlstr = "update kejadian_lain set departemen='$departemen', tanggal='$tanggal', jenis='$jenis', penanganan='$penanganan', uid='$uid', dlu='$dlu' where replid='$ref'";
	...
	function update_guru_bk($ref){
		...
		try {
			...
			$sqlstr = "update guru_bk set departemen='$departemen', idtahunajaran='$idtahunajaran', idguru='$idguru', aktif='$aktif', uid='$uid', dlu='$dlu' where replid='$ref'";
			...
			for ($x=0; $x<$datakelas; $x++) {
				...
				for ($i=0; $i<=$jmldata; $i++) {
					...
					if ($pilihkelas == 1) { 	
						...
						$sqlstr = "select replid from guru_penugasan where idguru='$idpegawai' and idtahunajaran='$idtahunajaran' and idtingkat='$idtingkat' and idkelas='$idkelas' and departemen='$departemen'";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
						$rows=$sql->rowCount();
	...
	function update_gugus($ref){
		...
		try {
			...
			$sqlstr = "update gugus set gugus='$gugus', departemen='$departemen', departemen_sekolah_id='$departemen_sekolah_id', kapasitas='$kapasitas', kapasitas_l='$kapasitas_l', kapasitas_p='$kapasitas_p', uid='$uid', dlu='$dlu' where replid='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();		
			...
			for ($i=0; $i<=$jmldata; $i++) {
				...
				if ( !empty($invoice_no) ) {	
					...
					if($num > 0) {
						...
						$sqlstr="insert into receipt_detail (ref, invoice_no, invoice_date, invoice_due_date, departemen, departemen_sekolah_id, discount, amount_paid, invoice_currency_code, invoice_rate, ref_type, amount_due, amount, line) values ('$ref', '$invoice_no', '$invoice_date', '$invoice_due_date', '$departemen', '$departemen_sekolah_id', '$discount', '$amount_paid', '$currency_code', '$rate', '$ref_type', '$amount_due', '$amount', $line)";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
	...
	//------update kunjungan_perpustakaan 
	function update_kunjungan_perpustakaan($ref){
		...
		try {
			...
			$sqlstr = "update kunjungan_perpustakaan set tanggal='$tanggal', jam='$jam', idsiswa='$idsiswa', idtingkat='$idtingkat', idkelas='$idkelas', keterangan='$keterangan', uid='$uid', dlu='$dlu' where replid='$ref'";
	...
	//-----update surat masuk disposisi
	function update_surat_masuk_disposisi($ref){
		$dbpdo = DB::create();
		...
		try {
			...
			for ($i=0; $i<$jmldata; $i++) {
				$delete 		= numberreplace($_POST['delete_'.$i.'']);
				$old_line 		= $_POST['old_line_'.$i.''];

				$disposisi_id 	= $_POST['disposisi_id_'.$i.''];
				$nama_disposisi = petikreplace($_POST['nama_disposisi_'.$i.'']);
				$instruksi 		= petikreplace($_POST['instruksi_'.$i.'']);

				$sqlstr = "select ref from surat_masuk_disposisi where ref='$ref' and line='$old_line'";
				$sql=$dbpdo->prepare($sqlstr);
				$sql->execute();
				$rows=$sql->rowCount();
				...
				if($rows > 0) {
					if($delete == 0) {
						$sqlstr = "update surat_masuk_disposisi set disposisi_id='$disposisi_id', nama_disposisi='$nama_disposisi', instruksi='$instruksi' where ref='$ref' and line='$old_line'";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
					} else {
						$sqlstr = "delete from surat_masuk_disposisi where ref='$ref' and line='$old_line'";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
					}
				} else {
					if ($nama_disposisi !='') { 				
						$line = maxline('surat_masuk_disposisi', 'line', 'ref', $ref, '');

						$sqlstr = "insert into surat_masuk_disposisi (ref, disposisi_id, nama_disposisi, instruksi, line) values ('$ref', '$disposisi_id', '$nama_disposisi', '$instruksi', '$line')";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
					}
				}
	...
	//-----update pinjam_alat
	function update_pinjam_alat($ref){	
		$dbpdo = DB::create();
		...
		try {
			...
			$sqlstr="update goods_loan set date='$date', receiver='$receiver', need='$need', memo='$memo', uid='$uid', dlu='$dlu' where ref='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
			...
			for ($i=0; $i<$jmldata; $i++) {
				...
				$sqlstr="select ref from goods_loan_detail where ref='$ref' and line='$old_line'";
				$sql=$dbpdo->prepare($sqlstr);
				$sql->execute();
				$rows = $sql->rowCount();
				...
				if($rows > 0) {
					if($delete == 0) {
						$sqlstr="update goods_loan_detail set qty='$qty' where ref='$ref' and line='$old_line'";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
					} else {
						$sqlstr="delete from goods_loan_detail where ref='$ref' and line='$old_line'";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
					}
				} else {			
	...
	//-----return pinjam_alat
	function kembali_pinjam_alat($ref){	
		$dbpdo = DB::create();
		...
		try {
			...
			$date_return		=	date("Y-m-d", strtotime($_POST["date_return"]));	
			$status				= 	'C';
			$employee_id		=	petikreplace($_POST["employee_id"]);
			
			$uid			=	$_SESSION["loginname"];
			$dlu			=	date("Y-m-d H:i:s");
						
			$sqlstr="update goods_loan set date_return='$date_return', status='C', employee_id='$employee_id', uid_return='$uid', dlu_return='$dlu' where ref='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
			
			//----------insert item detail
			$sqlstr="update goods_loan_detail set status='C' where ref='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	//-----return pinjam_ruang
	function selesai_peminjaman_room($ref){	
		$dbpdo = DB::create();
		...
		try {
			...
			$checkout_date		=	date("Y-m-d", strtotime($_POST["checkout_date"]));	
			$status				= 	'C';
			$checkout_uid		=	petikreplace($_POST["checkout_uid"]);
			
			$uid			=	$_SESSION["loginname"];
			$dlu			=	date("Y-m-d H:i:s");
						
			$sqlstr="update room_registration set checkout=1, checkout_date='$checkout_date', checkout_uid='$checkout_uid' where ref='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function update_rpt_absensi_harian_siswa($ref){	
		$dbpdo = DB::create();
		...
		try {
			...
			$sqlstr="update phsiswa set ijin='$ijin', sakit='$sakit', cuti='$cuti', alpa='$alpa', keterangan='$keterangan', ts='$ts' where replid='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	//-----update registrasi (Verif)
	function update_registrasi_verif($ref){
		...
		try {
			...
			$sqlstr="update calonsiswa set verifikasi=1, verif_uid='$uid', verif_dlu='$dlu' where replid='$ref' ";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	//-----update registrasi (Unverif)
	function update_registrasi_unverif($ref){
		...
		try {
			...
			$sqlstr="update calonsiswa set verifikasi=0, unverif_uid='$uid', unverif_dlu='$dlu' where replid='$ref' ";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	//---update_registrasi_lulus
	function update_registrasi_lulus(){
		
		$dbpdo = DB::create();
		
		$pilihall 		=	$_POST["pilihall"];
		$uid			=	$_SESSION["loginname"];
		$dlu			=	date("Y-m-d H:i:s");
		...
		for($i=0; $i<=$jmldata; $i++) {
			...
			if($select == 1) {
				$ref 			= $_POST['ref_'.$i.''];				
				//update lulus
				$sqlstr="update calonsiswa set lulus=1, lulus_uid='$uid', lulus_dlu='$dlu' where replid='$ref'"; 
		...
		if($pilihall == 1) {
			$sqlstr="update calonsiswa set lulus=1, lulus_uid='$uid', lulus_dlu='$dlu' where ifnull(lulus,0)=0"; 
			$sql=$dbpdo->query($sqlstr);
		}		
	...
	//-----update_registrasi_unlulus
	function update_registrasi_unlulus($ref){
		...
		try {
			...
			$sqlstr="update calonsiswa set lulus=0, lulus_uid='', lulus_dlu='00:00:00' where replid='$ref' ";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	//-----update deskripsi raport P5
	function update_deskripsi_raport_p5($id){
		$dbpdo = DB::create();
		...
		try {
			...
			for($i=0; $i<=$jmldata; $i++) {
				...
				$jmldata_elemen =	petikreplace($_POST['jmldata_elemen_'.$i.'']);

				if($delete == 0) {
					$sqlstr = "select replid from deskripsi_raport_p5_dimensi where deskripsi_p5_id='$id' and line='$old_line'";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
					$rows = $sql->rowCount();
					$datadimensi = $sql->fetch(PDO::FETCH_OBJ);
					$deskripsi_p5_dimensi_id = $datadimensi->replid;

					if($rows > 0) {
						$sqlstr = "update deskripsi_raport_p5_dimensi set dimensi='$dimensi' where deskripsi_p5_id='$id' and line='$old_line'";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();

						##data elemen
						for($j=0; $j<=$jmldata_elemen; $j++) {
							$delete_elemen		=	$_POST['delete_elemen_'.$i.$j.''];
							$elemen_id 			=	$_POST['elemen_id_'.$i.$j.''];
							$elemen_old_line	=	$_POST['elemen_old_line_'.$i.$j.''];
							$elemen				=	petikreplace($_POST['elemen_'.$i.$j.'']);

							if($delete_elemen == 0) {
								$sqlstr = "select replid from deskripsi_raport_p5_elemen where deskripsi_p5_dimensi_id='$deskripsi_p5_dimensi_id' and line='$elemen_old_line'";
								$sqlel=$dbpdo->prepare($sqlstr);
								$sqlel->execute();
								$rowels = $sqlel->rowCount();
								if($rowels > 0) {
									$sqlstr = "update deskripsi_raport_p5_elemen set elemen='$elemen' where deskripsi_p5_dimensi_id='$deskripsi_p5_dimensi_id' and line='$elemen_old_line'";
									$sql=$dbpdo->prepare($sqlstr);
									$sql->execute();
								} else {
									if($elemen != "") {
										$line_elemen = maxline('deskripsi_raport_p5_elemen', 'line', 'deskripsi_p5_dimensi_id', $deskripsi_p5_dimensi_id, '');
										$sqlstr = "insert into deskripsi_raport_p5_elemen(deskripsi_p5_dimensi_id, elemen, line) values ('$deskripsi_p5_dimensi_id', '$elemen', '$line_elemen')";
										$sql=$dbpdo->prepare($sqlstr);
										$sql->execute();
									}
								}
								...
								if($delete_elemen == 1) {
									$sqlstr = "delete from deskripsi_raport_p5_elemen where deskripsi_p5_dimensi_id='$deskripsi_p5_dimensi_id' and line='$elemen_old_line'";
									$sql=$dbpdo->prepare($sqlstr);
									$sql->execute();
								}
							}
						...
						if($dimensi != "") {
							$line = maxline('deskripsi_raport_p5_dimensi', 'line', 'deskripsi_p5_id', $id, '');
						
							$sqlstr = "insert into deskripsi_raport_p5_dimensi (deskripsi_p5_id, dimensi, line) values ('$id', '$dimensi', '$line')";
							$sql=$dbpdo->prepare($sqlstr);
							$sql->execute();
							...
							$results			=	$dbpdo->query($sqlstr);
							$data 				=  	$results->fetch(PDO::FETCH_OBJ);
							$deskripsi_p5_dimensi_id	=	$data->last_id;

							##data elemen
							$elemen 		=	petikreplace($_POST['elemen_'.$i.'']);
							if($elemen != "") {
								$line_elemen = maxline('deskripsi_raport_p5_elemen', 'line', 'deskripsi_p5_dimensi_id', $deskripsi_p5_dimensi_id, '');
								$sqlstr = "insert into deskripsi_raport_p5_elemen(deskripsi_p5_dimensi_id, elemen, line) values ('$deskripsi_p5_dimensi_id', '$elemen', '$line_elemen')";
								$sql=$dbpdo->prepare($sqlstr);
					...
					$sql->execute();
					$data = $sql->fetch(PDO::FETCH_OBJ);
					$deskripsi_p5_dimensi_id = $data->replid;

					$sqlstr = "delete from deskripsi_raport_p5_elemen where deskripsi_p5_dimensi_id='$deskripsi_p5_dimensi_id'";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();

					// $sqlstr = "delete from deskripsi_raport_p5_nilai where deskripsi_p5_dimensi_id='$deskripsi_p5_dimensi_id'";
					// $sql=$dbpdo->prepare($sqlstr);
					// $sql->execute();

					$sqlstr = "delete from deskripsi_raport_p5_dimensi where deskripsi_p5_id='$id' and line='$old_line'";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
				}
			...
			$sqlstr = "update deskripsi_raport_p5 set departemen='$departemen', departemen_sekolah_id='$departemen_sekolah_id', tahunajaran_id='$tahunajaran_id', tingkat_id='$tingkat_id', semester_id='$semester_id', guru_id='$guru_id', tema='$tema', aktif='$aktif', uid='$uid', dlu='$dlu' where replid='$id'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	//-----update raport_p5
	function update_raport_p5($idtema){	
		$dbpdo = DB::create();
		...
		try {
			...
			$jmldata = numberreplace($_POST['jmldata']);	
			for ($i=0; $i<$jmldata; $i++) {
				if (isset($_POST['pilih']) && is_array($_POST['pilih'])) {
			        $selected_options = $_POST['pilih'];
			        foreach ($selected_options as $pilih) {
			        	if($pilih) {
			        		$deskripsi_p5_id 	= numberreplace($_POST['raport_p5_id_'.$i.'']);
			        		if($pilih == $deskripsi_p5_id) {
								...
								if($idsiswa_1 != "") {
									if(!empty($idsiswa_1)) {
										for($q=0; $q<count($idsiswa_1); $q++) {
											...
											$sqlstr="select a.replid from raport_p5 a where a.syscode = '$idtema' and a.idsiswa='$idsiswa' ";
											$sql=$dbpdo->prepare($sqlstr);
											$sql->execute();
											$rows_det = $sql->rowCount();

											if($rows_det = 0) {
												$sqlstr="insert into raport_p5 (departemen, departemen_sekolah_id, idtingkat, idkelas, idtahunajaran, idsemester, idsiswa, deskripsi_p5_id, idguru, tema, uid, dlu) values ('$departemen', '$departemen_sekolah_id', '$idtingkat', '$idkelas', '$idtahunajaran', '$idsemester', '$idsiswa', '$deskripsi_p5_id', '$idguru', '$tema', '$uid', '$dlu')";
												$sql=$dbpdo->prepare($sqlstr);
												$sql->execute();
											} else {
												$sqlstr="update raport_p5 set departemen='$departemen', departemen_sekolah_id='$departemen_sekolah_id', deskripsi_p5_id='$deskripsi_p5_id', idguru='$idguru', tema='$tema', uid='$uid', dlu='$dlu' where syscode = '$idtema' and idsiswa='$idsiswa' ";
												$sql=$dbpdo->prepare($sqlstr);
												$sql->execute();
											}
										}
	...
	//-----update setup periode raport p5
	function update_setup_periode_raport_p5($ref){
		$dbpdo = DB::create();
		...
		try {
			...
			$sqlstr = "update setup_periode_raport_p5 set departemen_id='$departemen_id', departemen_sekolah_id='$departemen_sekolah_id', idtahunajaran='$idtahunajaran', semester_id='$semester_id', tingkat_id='$tingkat_id', tanggal_ttd='$tanggal_ttd', aktif='$aktif', uid='$uid', dlu='$dlu' where replid='$ref'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();		
		}
	...
	function update_presensiharian_setup($replid){
		...
		try {
			...
			$sqlstr = "update presensiharian_setup set departemen='$departemen', departemen_sekolah_id='$departemen_sekolah_id', idtingkat='$idtingkat', idkelas='$idkelas', jam_masuk='$jam_masuk', jam_terlambat='$jam_terlambat', jam_pulang_cepat='$jam_pulang_cepat', jam_pulang='$jam_pulang', aktif='$aktif', uid='$uid', dlu='$dlu' where replid='$replid'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();	
