...
class insert{
	...
	function insert_usr($ref,$photo){
		...
		try {
			...
			//----------insert user departemen
			$usr_jmldata3 = (empty($_POST['jmldata3'])) ? 0 : $_POST['jmldata3'];			
			for ($i=1; $i<=$usr_jmldata3; $i++) {
				$slc_unit = (empty($_POST['slc_unit_'.$i.''])) ? 0 : $_POST['slc_unit_'.$i.''];
				
				if ($slc_unit==1) { 				
					$departemen_id = $_POST['departemen_id_'.$i.''];
					$departemen_dekolah_id = $_POST['departemen_sekolah_id_'.$i.''];
								
					$sqlstr = "insert into usr_departemen (usrid, departemen_id, departemen_sekolah_id)
						values
						(
							'".$usrid."',
							'".$departemen_id."',
							'".$departemen_dekolah_id."'
						)";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
	...
	//-----insert registrasi simple
	function insert_registrasi_simple($ref){
		$dbpdo = DB::create();
		
		try {
			
			$tanggal			=	date("Y-m-d", strtotime($_POST["tanggal"]));
			$tahunmasuk 		=	date("Y", strtotime($_POST["tanggal"]));
			...
			$kelamin 			=	$_POST["kelamin"];
			$nik				=	$_POST["nik"];
			$alamatsiswa		=	petikreplace($_POST["alamatsiswa"]);
			$kodepossiswa		=	$_POST["kodepossiswa"];
			$telponsiswa		=	$_POST["telponsiswa"];
			$hpsiswa			=	$_POST["hpsiswa"];
			$emailsiswa			=	$_POST["emailsiswa"];
			$departemen_sekolah_id = numberreplace($_POST["departemen_sekolah_id"]);
			...
			if($foto_file != "") {			
				$foto_file = $nopendaftaran . '_' . $foto_file;					
				$uploadfile = $uploaddir . $foto_file;		
				if (move_uploaded_file($_FILES['foto_file']['tmp_name'], $uploadfile)) {
					echo "";											
				} 
			...
			$sqlstr = "insert into calonsiswa (departemen, nopendaftaran, tanggal, idproses, idkelompok, foto_file, nama, panggilan, tmplahir, tgllahir, kelamin, nik, agama, alamatsiswa, kodepossiswa, hpsiswa, telponsiswa, emailsiswa, tahunmasuk, departemen_sekolah_id, token, uid, dlu) values ('$departemen', '$nopendaftaran', '$tanggal', '$idproses', '$idkelompok', '$foto_file', '$nama', '$panggilan', '$tmplahir', '$tgllahir', '$kelamin', '$nik', '$agama', '$alamatsiswa', '$kodepossiswa', '$hpsiswa', '$telponsiswa', '$emailsiswa', '$tahunmasuk', '$departemen_sekolah_id', '$token', '$uid', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_departemen(){
		...
		try {
			...
			$results		=	$dbpdo->query($sqlstr);
			$data 			=  	$results->fetch(PDO::FETCH_OBJ);
			$departemen_id	=	$data->last_id;

			$jmldata 	=	numberreplace($_POST["jmldata"]);
			for($i=0; $i<$jmldata; $i++) {
				$nama 		=	petikreplace($_POST['nama_'.$i.'']);
				$alamat 	=	petikreplace($_POST['alamat_'.$i.'']);
				$nipkepsek	=	$_POST['nipkepsek_'.$i.''];
				$kuota		=	numberreplace($_POST['kuota_'.$i.'']);
				$tanggal_awal 	=	date('Y-m-d H:i', strtotime($_POST['tanggal_awal_'.$i.'']));
				$tanggal_akhir 	=	date('Y-m-d H:i', strtotime($_POST['tanggal_akhir_'.$i.'']));
				$biaya		=	numberreplace($_POST['biaya_'.$i.'']);
				$bank		=	$_POST['bank_'.$i.''];
				$no_rekening=	$_POST['no_rekening_'.$i.''];
				$nama_pemilik	=	$_POST['nama_pemilik_'.$i.''];

				if($nama != "") {
					$sqlstr = "insert into departemen_sekolah (departemen_id, nama, alamat, nipkepsek, kuota, tanggal_awal, tanggal_akhir, biaya, bank, no_rekening, nama_pemilik) values ('$departemen_id', '$nama', '$alamat', '$nipkepsek', '$kuota', '$tanggal_awal', '$tanggal_akhir', '$biaya', '$bank', '$no_rekening', '$nama_pemilik')";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
				    ...
				    $url__ = explode("/", $url);
				    $link = $url__[0].'//'.$url__[2].'/reg/registrasi?unit=';
				    //$link = $url__[0].'//'.$url__[2].'/'.$url__[3].'/reg/registrasi?unit=';

					$link_registrasi = $link.$sekolah_id;
					$sqlstr = "update departemen_sekolah set link_registrasi='$link_registrasi' where departemen_id='$departemen_id' and replid='$sekolah_id'";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
				}
			}
	...
	function insert_tingkat(){
		...
		try {
			...
			$sqlstr = "insert into tingkat (tingkat, departemen, departemen_sekolah_id, aktif, keterangan, urutan, tanggal_ttd, ts) values ('$tingkat', '$departemen', '$departemen_sekolah_id', '$aktif', '$keterangan', '$urutan', '$tanggal_ttd', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_kelas(){
		...
		try {
			...
			$sqlstr = "insert into kelas (departemen_sekolah_id, idtahunajaran, idtingkat, kelas, kapasitas, nipwali, aktif, keterangan, ttd_file_hd, ts) values ('$departemen_sekolah_id', '$idtahunajaran', '$idtingkat', '$kelas', '$kapasitas', '$nipwali', '$aktif', '$keterangan', '$ttd_file_hd_a', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_tahunajaran(){
		...
		try {
			...
			$departemen			=	$_POST['departemen'];
			$info1				=	$_POST["info1"];		
			$info2				=	numberreplace($_POST["info2"]);	
			$dlu				=	date("Y-m-d H:i:s");
			...
			$sqlstr = "insert into tahunajaran (tahunajaran, departemen, tglmulai, tglakhir, aktif, info1, info2, info3, keterangan, ts) values ('$tahunajaran', '$departemen', '$tglmulai', '$tglakhir', '$aktif', '$info1', '$info2', '$ttd_file_a', '$keterangan', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
			...
			$sqlstr = "update tahunajaran set info2='0' where replid<>'$lastid'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
		}
	...
	function insert_pustaka(){
		...
		try {
			...
			//-----------file_ebook-------
			$uploaddir 		= 'app/file_ebook/';
			$file_ebook2	= $_POST["file_ebook2"];
			$file_ebook 	= $_FILES['file_ebook']['name']; 
			$tmpname  		= $_FILES['file_ebook']['tmp_name'];
			$filesize 		= $_FILES['file_ebook']['size'];
			$filetype 		= $_FILES['file_ebook']['type'];
			
			if (empty($file_ebook)) { 
				$file_ebook = $file_ebook2; 
			} 
			
			if($file_ebook != "") {	
				
				if($file_ebook != $file_ebook2) {
				
					if(!empty($file_ebook2)) {
						unlink($uploaddir . $file_ebook2); //remove file 
					}
					
					$file_ebook = $format . '_' . $file_ebook;
				}
							
				$uploadfile = $uploaddir . $file_ebook;		
				if (move_uploaded_file($_FILES['file_ebook']['tmp_name'], $uploadfile)) {
					echo "";											
				} 
			}
			//----------------	
	...
	function insert_kenaikan($idkelas, $idkelas2){
		...
		try {
			...
			$idangkatan2 = numberreplace($_POST["idangkatan2"]);
			$mulai	=	date("Y-m-d");
			$ts		=	date("Y-m-d H:i:s");
	...
	function insert_pegawai(){
		...
		try {
			...
			$jabatan_id 		=	numberreplace($_POST["jabatan_id"]);
			$nip				=	$_POST["nip"];
			$nrk				=	$_POST["nrk"];
			$nama				=	petikreplace($_POST["nama"]);
			$panggilan			=	petikreplace($_POST["panggilan"]);
			$pangkat_id 		=	numberreplace($_POST["pangkat_id"]);
			$kelamin			=	$_POST["kelamin"];
			$gelar				=	$_POST["gelar"];
            ...
            $pendidikan_terakhir_id = numberreplace($_POST["pendidikan_terakhir_id"]);
            $jurusan 			=	petikreplace($_POST["jurusan"]);
            $tahun_lulus 		=	$_POST["tahun_lulus"];
            $universitas 		=	petikreplace($_POST["universitas"]);
			$noid				=	$_POST["noid"];
			$alamat				=	petikreplace($_POST["alamat"]);
			$telpon				=	$_POST["telpon"];
			...
			$idjenis_sertifikasi=	petikreplace($_POST["idjenis_sertifikasi"]);
			$npwp				=	$_POST["npwp"];
			$nuptk				=	$_POST["nuptk"];
			...
			$usia				=	numberreplace($_POST["usia"]);
			$ajar_lain			=	$_POST["ajar_lain"];
			$jumlah_jam_induk 	=	numberreplace($_POST["jumlah_jam_induk"]);
			...
			$sqlstr = "insert into pegawai (bagian, nip, nrk, nama, panggilan, kelamin, gelar, gelar_belakang, tmplahir, tgllahir, agama, suku, jabatan_id, pangkat_id, nikah, jenis_id, pendidikan_terakhir_id, jurusan, tahun_lulus, universitas, noid, alamat, telpon, handphone, email, karpeg, no_sertifikasi, idjenis_sertifikasi, npwp, nuptk, tmt_cpns, unit_cpns, no_sk_masuk, idstatus_pegawai, nik, nama_ibu, nama_pasangan, tempat_lahir_pasangan, tanggal_lahir_pasangan, tanggal_nikah, tempat_nikah, pekerjaan_pasangan, instansi_pasangan, nip_pasangan, keluarga_tanggungan, usia, ajar_lain, jumlah_jam_induk, jumlah_jam_ajar_lain, nama_bank, unit_bank, no_rek, nama_pemilik, desa, kecamatan, kode_pos, tanggal_pensiun, foto_file, keterangan, no_sk_tetap, tanggal_sk_tetap, ts) values ('$bagian', '$nip', '$nrk', '$nama', '$panggilan', '$kelamin', '$gelar', '$gelar_belakang', '$tmplahir', '$tgllahir', '$agama', '$suku', '$jabatan_id', '$pangkat_id', '$nikah', '$jenis_id', '$pendidikan_terakhir_id', '$jurusan', '$tahun_lulus', '$universitas', '$noid', '$alamat', '$telpon', '$handphone', '$email', '$karpeg', '$no_sertifikasi', '$idjenis_sertifikasi', '$npwp', '$nuptk', '$tmt_cpns', '$unit_cpns', '$no_sk_masuk', '$idstatus_pegawai', '$nik', '$nama_ibu', '$nama_pasangan', '$tempat_lahir_pasangan', '$tanggal_lahir_pasangan', '$tanggal_nikah', '$tempat_nikah', '$pekerjaan_pasangan', '$instansi_pasangan', '$nip_pasangan', '$keluarga_tanggungan', '$usia', '$ajar_lain', '$jumlah_jam_induk', '$jumlah_jam_ajar_lain', '$nama_bank', '$unit_bank', '$no_rek', '$nama_pemilik', '$desa', '$kecamatan', '$kode_pos', '$tanggal_pensiun', '$foto_file', '$keterangan', '$no_sk_tetap', '$tanggal_sk_tetap', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	//-----insert pegawai non pns
	function insert_pegawai_non_pns(){
		$dbpdo = DB::create();
		...
		try {
			...
			$no_sk_masuk		=	$_POST["no_sk_masuk"];			
			$idstatus_pegawai	=	(empty($_POST["idstatus_pegawai"])) ? 0 : $_POST["idstatus_pegawai"];
			$no_skkki			=	$_POST["no_skkki"];		
			$tahun_skkki		=	$_POST["tahun_skkki"];		
			$nik				=	$_POST["nik"];
			$nama_ibu			=	$_POST["nama_ibu"];
			$nama_pasangan		=	$_POST["nama_pasangan"];
			$tempat_lahir_pasangan	=	$_POST["tempat_lahir_pasangan"];
			...
			$sqlstr = "insert into pegawai (bagian, jabatan_id, nip, nama, panggilan, kelamin, gelar, gelar_belakang, tmplahir, tgllahir, agama, suku, nikah, jenis_id, noid, alamat, telpon, handphone, email, karpeg, no_sertifikasi, idjenis_sertifikasi, pendidikan_terakhir_id, jurusan, tahun_lulus, universitas, npwp, nuptk, tmt_cpns, unit_cpns, no_sk_masuk, no_skkki, tahun_skkki, idstatus_pegawai, nik, nama_ibu, nama_pasangan, tempat_lahir_pasangan, tanggal_lahir_pasangan, tanggal_nikah, tempat_nikah, pekerjaan_pasangan, instansi_pasangan, nip_pasangan, keluarga_tanggungan, usia, ajar_lain, jumlah_jam_induk, jumlah_jam_ajar_lain, nama_bank, unit_bank, no_rek, nama_pemilik, desa, kecamatan, kode_pos, tanggal_pensiun, foto_file, keterangan, no_sk_tetap, tanggal_sk_tetap, info1, ts) values ('$bagian', '$jabatan_id', '$nip', '$nama', '$panggilan', '$kelamin', '$gelar', '$gelar_belakang', '$tmplahir', '$tgllahir', '$agama', '$suku', '$nikah', '$jenis_id', '$noid', '$alamat', '$telpon', '$handphone', '$email', '$karpeg', '$no_sertifikasi', '$idjenis_sertifikasi', '$pendidikan_terakhir_id', '$jurusan', '$tahun_lulus', '$universitas', '$npwp', '$nuptk', '$tmt_cpns', '$unit_cpns', '$no_sk_masuk', '$no_skkki', '$tahun_skkki', '$idstatus_pegawai', '$nik', '$nama_ibu', '$nama_pasangan', '$tempat_lahir_pasangan', '$tanggal_lahir_pasangan', '$tanggal_nikah', '$tempat_nikah', '$pekerjaan_pasangan', '$instansi_pasangan', '$nip_pasangan', '$keluarga_tanggungan', '$usia', '$ajar_lain', '$jumlah_jam_induk', '$jumlah_jam_ajar_lain', '$nama_bank', '$unit_bank', '$no_rek', '$nama_pemilik', '$desa', '$kecamatan', '$kode_pos', '$tanggal_pensiun', '$foto_file', '$keterangan', '$no_sk_tetap', '$tanggal_sk_tetap', '1', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_izin_siswa($replid){
		...
		try {
			...
			$idsiswa	    =	(empty($_POST["idsiswa"])) ? 0 : $_POST["idsiswa"];
            $jam            =   date('Y-m-d', strtotime($_POST["jam"]));
            $menit          =   date('H:i', strtotime($_POST["jam"]));
            $jam            =   $jam . ":" . $menit;
	...
	function insert_pelajaran(){
		...
		try {
			...
			$merdeka			=	numberreplace($_POST["merdeka"]);
			$uid				=	$_SESSION["loginname"];
			$dlu				=	date("Y-m-d H:i:s");
			
			$sqlstr = "insert into pelajaran (kode, nama, departemen, departemen_sekolah_id, sifat, ekstra_sifat, pegawai_id, aktif, keterangan, info1, info2, info3, alias, idpelajaran_alias, minat, bentrok, merdeka, token, ts) values ('$kode', '$nama', '$departemen', '$departemen_sekolah_id', '$sifat', '$ekstra_sifat', '$pegawai_id', '$aktif', '$keterangan', '$sks', '$kkm_p', '$uid', '$alias', '$idpelajaran_alias', '$minat', '$bentrok', '$merdeka', '$kkm_k', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_dasarpenilaian(){
		...
		try {
			...
			$sqlstr = "insert into dasarpenilaian (departemen, dasarpenilaian, keterangan, ts) values ('$departemen', '$dasarpenilaian', '$keterangan', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_kompetensi(){
		...
		try {
			...
			$sqlstr = "insert into kompetensi (departemen, kode, kompetensi, aktif, dlu) values ('$departemen', '$kode', '$kompetensi', '$aktif', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_assesmen_observasi($ref){
		...
		try {
			...
			for($x=0; $x<=$jumlah; $x++) {
				$idaspek_perkembangan	=	numberreplace($_POST['idaspek_perkembangan_'.$x.'']);
		...
		try {
			...
			for($y=0; $y<$jmldata_jenis_aspek; $y++) {
				...
				for($z=0; $z<$jml_aspek_detail; $z++) {
					...
					$nilai				=	numberreplace($_POST['nilai_'.$y.$z.'']);
	...
	//-----insert kelulusan
	function insert_kelulusan($idkelas, $idkelas2, $tgllulus, $departemen, $departemen_sekolah_id){
		$dbpdo = DB::create();
		...
		try {
			...
			for ($x=0; $x<=$jmldata; $x++) {
				...
				if($lulus > 0 && $nis != "") {
					...
					//insert alumni
					$sqlstr="insert into alumni set nis='$nis', tgllulus='$tgllulus', tktakhir='$idtingkat', klsakhir='$idkelas', departemen='$departemen', departemen_sekolah_id='$departemen_sekolah_id', keterangan='$keterangan', info3='$uid'";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
	...
	function insert_kartu_rencana_studi(){
		...
		try {
			...
			$sqlstr = "insert into kartu_rencana_studi (departemen, peminatan, tingkat_id, kelompok_pelajaran_id, pelajaran_kode, pelajaran_id, semester_id, sks, idtahunajaran, urutan, kode1, kode2, uid, dlu) values ('$departemen', '$peminatan', '$tingkat_id', '$kelompok_pelajaran_id', '$pelajaran_kode', '$pelajaran_id', '$semester_id', '$sks', '$idtahunajaran', '$urutan', '$kode1', '$kode2', '$uid', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_pemetaan_kd(){
		...
		try {
			...
			$sqlstr = "insert into pemetaan_kd (departemen, permen_kd_id, idtingkat, idpelajaran, kd, kode, uraian, kkm_sekolah, kkm_pelajaran, jumlah_ukbm, urutan, aktif, uid, dlu) values ('$departemen', '$permen_kd_id', '$idtingkat', '$idpelajaran', '$kd', '$kode', '$uraian', '$kkm_sekolah', '$kkm_pelajaran', '$jumlah_ukbm', '$urutan', '$aktif', '$uid', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_pemetaan_kd_siswa(){
		...
		try {
			...
			$sqlstr = "insert into pemetaan_kd_siswa (departemen, pemetaan_kd_id, idsiswa, idguru, idpelajaran, nilai, uid, dlu) values ('$departemen', '$pemetaan_kd_id', '$idsiswa', '$idguru', '$idpelajaran', '$nilai', '$uid', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_buku_kunjungan($ref){
		...
		try {
			...
			$telepon			=	petikreplace($_POST["telepon"]);
			...
			$sqlstr = "insert into buku_kunjungan (ref, tanggal, nama, instansi, telepon, ttd, keperluan, kesan_pesan, keterangan, uid, dlu) values ('$ref', '$tanggal', '$nama', '$instansi', '$telepon', '$ttd', '$keperluan', '$kesan_pesan', '$keterangan', '$uid', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_material($code){		
		...
		try {
			...
			$item_booking 	=	numberreplace($_POST["item_booking"]);
			$active			=	(empty($_POST["active"])) ? 0 : $_POST["active"];
			$uid			=	$_SESSION["loginname"];
			$dlu			=	date("Y-m-d H:i:s");
			$syscode		= 	random(25);
			...
			$sqlstr="insert into item (code, old_code, name, item_group_id, item_subgroup_id, item_type_code, item_category_id, brand_id, size_id, uom_code_stock, uom_code_sales, uom_code_purchase, minimum_stock, maximum_stock, photo, consigned, item_booking, active, uid, dlu, syscode) values ('$code', '$old_code', '$name', '$item_group_id', '$item_subgroup_id', '$item_type_code', '$item_category_id', '$brand_id', '$size_id', '$uom_code_stock', '$uom_code_sales', '$uom_code_purchase', '$minimum_stock', '$maximum_stock', '$photo_a', '$consigned', '$item_booking', '$active', '$uid', '$dlu', '$syscode')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_room_registration($ref){
		...
		try {
			...
			$exp_room_ref = explode("|", $room_ref);

			$jmldata = count($exp_room_ref); 
			for ($i=0; $i<=$jmldata; $i++) {
				
				$room_ref 		= $exp_room_ref[$i];
	...
	function insert_siswa_terlambat(){
		...
		try {
			...
			$sqlstr = "insert into siswa_terlambat (tanggal, jam, idsiswa, idtingkat, idkelas, alasan, penanganan, uid, dlu) values ('$tanggal', '$jam', '$idsiswa', '$idtingkat', '$idkelas', '$alasan', '$penanganan', '$uid', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_siswa_izin(){
		...
		try {
			...
			if($idtingkat == 0 || $idtingkat == "") {
				$sqlstr = "select a.idkelas, c.departemen, b.idtingkat from siswa a left join kelas b on a.idkelas=b.replid left join tingkat c on b.idtingkat=c.replid where a.replid='$idsiswa' and a.aktif=1 limit 1";
				$sql=$dbpdo->prepare($sqlstr);
				$sql->execute();
				$data = $sql->fetch(PDO::FETCH_OBJ);
	...
	function insert_kejadian_lain(){
		...
		try {
			...
			$sqlstr = "insert into kejadian_lain (departemen, tanggal, jenis, penanganan, uid, dlu) values ('$departemen', '$tanggal', '$jenis', '$penanganan', '$uid', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_guru_bk(){
		...
		try {
			...
			$sqlstr = "insert into guru_bk (departemen, idtahunajaran, idguru, aktif, uid, dlu) values ('$departemen', '$idtahunajaran', '$idguru', '$aktif', '$uid', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	function insert_guru_penugasan(){
		...
		try {
			...
			$departemen 	=	$_POST["departemen"];
			$guru_penugasan =	$_POST["guru_penugasan"];
			$idtahunajaran	=	(empty($_POST["idtahunajaran"])) ? 0 : $_POST["idtahunajaran"];
			$idpegawai		=	(empty($_POST["idpegawai"])) ? 0 : $_POST["idpegawai"];
			$aktif			=	(empty($_POST["aktif"])) ? 0 : $_POST["aktif"];
			...
			for ($x=0; $x<$datakelas; $x++) {
				...
				for ($i=0; $i<=$jmldata; $i++) {
					...
					if ($pilihkelas == 1) { 	
						...
						$sqlstr = "select replid from guru_penugasan where departemen='$departemen' and idguru='$idpegawai' and idtahunajaran='$idtahunajaran' and idtingkat='$idtingkat' and idkelas='$idkelas'";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
						$rows=$sql->rowCount();
						
						if($rows == 0) {
							$sqlstr="insert into guru_penugasan (departemen, idtahunajaran, idguru, idtingkat, idkelas, aktif, uid, dlu) values ('$departemen', '$idtahunajaran', '$idpegawai', '$idtingkat', '$idkelas', '$aktif', '$uid', '$dlu')";
							$sql=$dbpdo->prepare($sqlstr);
							$sql->execute();	
						}
	...
	function insert_gugus(){
		...
		try {
			...
			$sqlstr = "insert into gugus (gugus, departemen, departemen_sekolah_id, kapasitas, kapasitas_l, kapasitas_p, uid, dlu) values ('$gugus', '$departemen', '$departemen_sekolah_id', '$kapasitas', '$kapasitas_l', '$kapasitas_p', '$uid', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
		...
		try {
			...
			for ($i=0; $i<$jmldata; $i++) {
				...
				if ( !empty($invoice_no) && $amount_paid <> 0 && $select == 1 ) {
					...
					$departemen 	=	$_POST["departemen_".$i.""];
					$departemen_sekolah_id = numberreplace($_POST["departemen_sekolah_id_".$i.""]);
					...
					$sqlstr="insert into receipt_detail (ref, invoice_no, invoice_date, invoice_due_date, discount, amount_paid, invoice_currency_code, invoice_rate, ref_type, departemen, departemen_sekolah_id, amount_due, amount, line) values ('$ref', '$invoice_no', '$invoice_date', '$invoice_due_date', '$discount', '$amount_paid', '$currency_code', '$rate', '$ref_type', '$departemen', '$departemen_sekolah_id', '$amount_due', '$amount', $line)";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
					
					//insert AR
					$sqlstr="insert into ar(ref, invoice_no, date, due_date, contact_type, contact_code, contact_other, departemen, departemen_sekolah_id, debit_amount, credit_amount, discount_amount, invoice_type, ref_type, currency_code, rate, exchange_type, exchange_date, top, description, uid, dlu, line) values('$ref', '$invoice_no', '$date', '$invoice_due_date', 'C', '$client_code', '', '$departemen', '$departemen_sekolah_id', 0, '$amount', '$discount', 'RCI', '$ref_type', '$currency_code', '$rate', '0', '00:00:00', 'C.O.D', '$memo', '$uid', '$dlu', '$line')";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
	...
	//-----insert kunjungan_perpustakaan
	function insert_kunjungan_perpustakaan(){
		$dbpdo = DB::create();
		...
		try {
			...
			$sqlstr = "insert into kunjungan_perpustakaan (tanggal, jam, idsiswa, idtingkat, idkelas, keterangan, uid, dlu) values ('$tanggal', '$jam', '$idsiswa', '$idtingkat', '$idkelas', '$keterangan', '$uid', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
	...
	//-----insert pinjam_alat
	function insert_pinjam_alat($ref){	
		$dbpdo = DB::create();
		...
		try {
			...
			$employee_id		=	(empty($_POST["employee_id"])) ? 0 : $_POST["employee_id"];
			$receiver		=	petikreplace($_POST["receiver"]);
			$memo			=	petikreplace($_POST["memo"]);
			$need			=	petikreplace($_POST["need"]);

			$from_time 		=	date("H:i", strtotime($_POST["from_time"]));
			$to_time 		=	date("H:i", strtotime($_POST["to_time"]));
			$booked			=	date("Y-m-d H:i", strtotime($_POST["booked"].' '.$from_time));	
			$booked_finish	=	date("Y-m-d H:i", strtotime($_POST["booked_finish"].' '.$to_time));	
			$uid			=	$_SESSION["loginname"];
			$dlu			=	date("Y-m-d H:i:s");
						
			$sqlstr="insert into goods_loan (ref, date, date_return, status, type, reason, form_no, warehouse_id_from, warehouse_id_to, employee_id, receiver, need, memo, booked, booked_finish, uid, dlu) values('$ref', '$date', '$date_return', '$status', '$type', '$reason', '$form_no', '$warehouse_id_from', '$warehouse_id_to', '$employee_id', '$receiver', '$need', '$memo', '$booked', '$booked_finish', '$uid', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
			
			//----------insert item detail
			$item_code = $_POST["item_code"];
			$exp_item_code = explode("|", $item_code);
			
			$qty = $_POST["qty"];
			$exp_qty = explode("|", $qty);

			$total = 0;
			$jmldata = count($exp_item_code); 			
			for ($i=0; $i<=$jmldata; $i++) {
				$item_code 		= $exp_item_code[$i];
				$qty 			= numberreplace($exp_qty[$i]);
				$uom_code 		= "pcs";
							
				if ( !empty($item_code) && !empty($uom_code) && $qty > 0) {				
					
					$sqlstr="select ref from goods_loan_detail where ref='$ref' and item_code='$item_code' and uom_code='$uom_code'";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
					$rows = $sql->rowCount();
					...
					if($rows > 0) {
						if($qty > 0) {
							$sqlstr="update goods_loan_detail set qty='$qty' where ref='$ref' and item_code='$item_code' and uom_code='$uom_code'";
							$sql=$dbpdo->prepare($sqlstr);
							$sql->execute();
						}
					} else {
						$line = maxline('goods_loan_detail', 'line', 'ref', $ref, '');
									
						$sqlstr="insert into goods_loan_detail (ref, item_code, uom_code, qty, line) values ('$ref', '$item_code', '$uom_code', '$qty', $line)";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
	...
	//-----insert mapel rapor
	function insert_generate_siswa_krs(){
		$dbpdo = DB::create();
		...
		try {
			...
			$sqlstr	= "select a.replid, a.peminatan, a.tingkat_id, a.kelompok_pelajaran_id, a.pelajaran_kode, a.pelajaran_id, a.semester_id, a.sks, a.idtahunajaran, a.urutan, a.kode1, a.kode2, a.uid, a.dlu, f.tahunajaran nama_tahunajaran, b.tingkat, c.nama nama_pelajaran, c.kode, d.semester from kartu_rencana_studi a left join tingkat b on a.tingkat_id=b.replid left join pelajaran c on a.pelajaran_id=c.replid left join semester d on a.semester_id=d.replid left join tahunajaran f on a.idtahunajaran=f.replid " . $where . " order by a.kelompok_pelajaran_id, a.urutan, a.replid";
			$sql1=$dbpdo->prepare($sqlstr);
			$sql1->execute();
			while($row_krs_detail=$sql1->fetch(PDO::FETCH_OBJ)) { 

				$semester_id 	= $row_krs_detail->semester_id;
				$pelajaran_id 	= $row_krs_detail->pelajaran_id;
				$kode 			= $row_krs_detail->kode;
				$pegawai_id 	= 0;
				$kelompok_pelajaran_id = $row_krs_detail->kelompok_pelajaran_id;
				$sks_id			= $row_krs_detail->sks;

				$where = " where b.idtingkat='$tingkat_id' ";
				if($idkelas != "") {
					$where = $where . " and a.idkelas='$idkelas' ";
				}
				$sqlstr="select a.nis, c.replid idpegawai from siswa a left join kelas b on a.idkelas=b.replid left join pegawai c on b.nipwali=c.nip ".$where;
				$sql2=$dbpdo->prepare($sqlstr);
				$sql2->execute();
				while($datasiswa=$sql2->fetch(PDO::FETCH_OBJ)) {
					$nis 		=	$datasiswa->nis;
					$pegawai_id	=	numberreplace($datasiswa->idpegawai);
					...
					if($rowcount == 0) {			
						$approved = 1;			
						$sqlstr = "insert into siswa_krs (nis, idtahunajaran, semester_id, pelajaran_id, kode, pegawai_id, kelompok_pelajaran_id, sks_id, approved, uid, dlu, ip_proxy, ip_pc, mobile, browser) values ('$nis', '$idtahunajaran', '$semester_id', '$pelajaran_id', '$kode', '$pegawai_id', '$kelompok_pelajaran_id', '$sks_id', '$approved', '$uid', '$dlu', '$ip_proxy', '$ip_pc', '$mobile', '$browser_cek')";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
					} else {
						$sqlstr = "update siswa_krs set kode='$kode', ip_proxy='$ip_proxy', ip_pc='$ip_pc', mobile='$mobile', browser='$browser' where nis='$nis' and semester_id='$semester_id' and pelajaran_id='$pelajaran_id' and idtahunajaran='$idtahunajaran'";
	...
	//-----return update_rpt_absensi_harian_siswa
	function insert_absensi_harian_siswa(){	
		$dbpdo = DB::create();
		...
		try {
			...
			$departemen 		=	$_POST["departemen"];
			$tanggal			=	date("Y-m-d", strtotime($_POST["tanggal"]));
			$jam				=	date("H:i", strtotime($_POST["jam"]));
			$ts 				=	date("Y-m-d H:i", strtotime($tanggal.' '.$jam));
			$idsiswa 			=	$_POST["idsiswa"];
			$idkelas 			=	$_POST["idkelas"];
			$keterangan 		=	petikreplace($_POST["keterangan"]);
			...
			if($rowsphs == 0) {
				$sqlstr		= "insert into phsiswa (idpresensi, nis, ijin, sakit, cuti, alpa, keterangan, ts, token, issync) values ('$idpresensi', '$nis', '$ijin', '$sakit', '$cuti', '$alpa', '$keterangan', '$dlu', 0, 0)";
				$sql=$dbpdo->prepare($sqlstr);
				$sql->execute();
			} else {
				$sqlstr		= "update phsiswa set ijin='$ijin', sakit='$sakit', cuti='$cuti', alpa='$alpa', keterangan='$keterangan', ts='$dlu' where nis='$nis' and idpresensi='$idpresensi'";
				$sql=$dbpdo->prepare($sqlstr);
				$sql->execute();
			}
	...
	//presensi general
	function insert_presensi_general(){
		...
		try {
			...
			$idpegawai_id		=	array();
			$idpegawai_id		=	explode("|", $_POST["idpegawai"]);
			
			$hadir_id		=	array();
			$hadir_id		=	explode("|", $_POST["hadir"]);
			
			$cuti_id	=	array();
			$cuti_id	=	explode("|", $_POST["cuti"]);
			...
			$jmldata		=	count($idpegawai_id); //(empty($_POST["jmldata"])) ? 0 : $_POST["jmldata"];
			$k=0;
			for($k=0; $k<$jmldata; $k++) {
				$idpegawai		=	$idpegawai_id[$k]; //$_POST[idsiswa_.$k];
				$absen			=	0; //$_POST[absen_.$k];
				$hadir 			= 	$hadir_id[$k];
				$cuti			=	$cuti_id[$k];
				$sakit			=	$sakit_id[$k];
				...
				$tanggal2 = date("Y-m-d", strtotime($_POST["tanggal"]));
				$sqlstr = "select replid from presensi_general where idpegawai='$idpegawai' and date_format(tanggal,'%Y-%m-%d')='$tanggal2'";
				$sql=$dbpdo->prepare($sqlstr);
				$sql->execute();
				$rows=$sql->rowCount();
				if($idpegawai != "") {
					if($rows == 0) {
						//replid	tanggal	idpegawai	hadir	ijin	sakit	alpa	cuti	keterangan	uid	dl
						$sqlstr = "insert into presensi_general (tanggal, idpegawai, hadir, cuti, sakit, ijin, alpa, uid, dlu) values ('$tanggal', '$idpegawai', '$hadir', '$cuti', '$sakit', '$izin', '$alpa', '$uid', '$dlu')";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();	
					} else {
						$sqlstr = "update presensi_general set hadir='$hadir', cuti='$cuti', sakit='$sakit', ijin='$izin', alpa='$alpa' where idpegawai='$idpegawai' and date_format(tanggal,'%Y-%m-%d')='$tanggal2'";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
					}
				}
	...
	//-----insert penempatan gugus
	function insert_penempatan_gugus(){
		$dbpdo = DB::create();
		...
		try {
			...
			for ($x=0; $x<$jmldata; $x++) {
				...
				if ($pilih == 1) {
					$sqlstr	=	"select a.replid, a.idproses, a.idkelompok, a.departemen, a.departemen_sekolah_id, a.tanggal, a.nopendaftaran, a.idtingkat, a.idjurusan, a.idminat, a.idminat1, a.foto_file, a.nama, a.panggilan, a.kelamin, a.nisn, a.nis, a.noijazah, a.tahunijazah, a.skhun, a.tahunskhun, a.noujian, a.nik, a.tmplahir, a.tgllahir, a.agama, a.kebutuhan_khusus, a.tahunmasuk, a.alamatsiswa, a.dusun, a.rt, a.rw, a.kelurahan, a.kodepossiswa, a.kecamatan, a.kabupaten, a.provinsi, a.transportasi, a.transportasi_kode, a.citacita, a.citacita_lain, a.idjenis_tinggal, a.jenis_tinggal, a.telponsiswa, a.hpsiswa, a.emailsiswa, a.kps, a.nokps, a.nokip, a.nokks, a.namaayah, a.tahunayah, a.alamatortu, a.kodeposortu, a.hportu, a.butuhkhususayah, a.butuhkhususketayah, a.pekerjaanayah, a.pekerjaanayah_lain, a.pendidikanayah, a.penghasilanayah, a.penghasilanayah_kode, a.namaibu, a.tahunibu, a.butuhkhususibu, a.butuhkhususketibu, a.pekerjaanibu, a.pekerjaanibu_lain, a.pendidikanibu, a.penghasilanibu, a.penghasilanibu_kode, a.wali, a.tahunwali, a.pekerjaanwali, a.pekerjaanwali_lain, a.pendidikanwali, a.penghasilanwali, a.tinggi, a.berat, a.jaraksekolah, a.jarak_km, a.waktutempuh, a.waktutempuh_menit, a.jsaudara, a.uid, a.dlu, b.tingkat, c.kelas jurusan, a.darah, a.file_darah, a.almayah, a.almibu, a.alamatibu, a.kodeposibu, a.hpibu, f.nama nama_sekolah from calonsiswa a left join tingkat b on a.idtingkat=b.replid left join kelas c on a.idjurusan=c.replid left join departemen_sekolah f on a.departemen_sekolah_id=f.replid where a.replid='$replid' order by a.replid ";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
					$data = $sql->fetch(PDO::FETCH_OBJ);
					$nik 						=	$data->nik;
					...
					$nama 						=	$data->nama;
					$panggilan 					=	petikreplace($data->panggilan);	
					$tglmasuk 					=	$data->tanggal;	
					$kelamin 					=	$data->kelamin;
					$tmplahir 					=	petikreplace($data->tmplahir);
					$tgllahir 					=	$data->tgllahir;
					$agama 						=	$data->agama;
					$warga 						=	"1";
					$jsaudara 					=	$data->jsaudara;
					$desa_kode					=	"";
					$kecamatan_kode 			=	"";
					$kota_kode 					=	"";
					$provinsi_kode 				=	"";
					...
					$desa 						=	"";
					$kecamatan 					=	"";
					$kodepossiswa 				=	$data->kodepossiswa;
					$jenistinggal		=	"";
					$kps				=	$data->kps;
					$nokps				=	$data->nokps;
					$kip				=	0;
					...
					$hportu				=	$data->hportu;
					$hpibu				=	$data->hpibu;
					$transportasi_kode	=	$data->transportasi_kode;
					$transportasi_lain	=	"";
					$jaraksekolah		=	numberreplace($data->jaraksekolah);
					$kesekolah			=	0;
					
					/*--------Keterangan kesehatan---------*/
					$berat				=	numberreplace($data->berat);
					$tinggi				=	numberreplace($data->tinggi);
					$kesehatan			=	"";
					$darah				=	$data->darah;
					$aktif 				=	1;

					$sqlstr = "select replid from angkatan where aktif=1 limit 1";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
					$dataangkatan = $sql->fetch(PDO::FETCH_OBJ);
					$idangkatan1 = $dataangkatan->replid;

					$sqlstr = "select replid from tahunajaran where aktif=1 limit 1";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
					$datata = $sql->fetch(PDO::FETCH_OBJ);
					$idangkatan = $datata->replid;
					...
					//-------insert siswa baru
					if($rows == 0) {
						$sqlstr = "insert into siswa (nik, idangkatan, idangkatan1, departemen, departemen_sekolah_id, nama, panggilan, tglmasuk, kelamin, tmplahir, tgllahir, agama, warga, jsaudara, desa_kode, kecamatan_kode, kota_kode, provinsi_kode, alamatsiswa, rt_siswa, rw_siswa, dusun, desa, kecamatan, kodepossiswa, jenistinggal, alamatortu, telponsiswa, hpsiswa, emailsiswa, telponortu, hportu, hpibu, transportasi_kode, kps, nokps, kip, nokip, namakip, nokks, no_akte_lahir, transportasi_lain, jaraksekolah, kesekolah, berat, tinggi, kesehatan, darah, file_darah, kelainan, asalsekolah_id, kota_asalsekolah, tglijazah, noijazah, tglskhun, skhun, noujian, nisnasal, nik_ayah, namaayah, nik_ibu, namaibu, tmplahirayah, tgllahirayah, tempat_bekerja_ayah, tmplahiribu, tgllahiribu, pekerjaanayah, pekerjaanibu, penghasilanayah_kode, penghasilanayah, penghasilanibu_kode, penghasilanibu, pendidikanayah, pendidikanibu, wnayah, wnibu, nik_wali, wali, tmplahirwali, tgllahirwali, pendidikanwali, pekerjaanwali, penghasilanwali_kode, penghasilanwali, tempat_bekerja_wali, alamatwali, hpwali, hubungansiswa, pekerjaanayah_lain, pekerjaanibu_lain, tempat_bekerja_ibu, pekerjaanwali_lain, rombel_id, nama_bank, no_rekening_bank, nama_pemilik_bank, pip, alasan_pip, idminat, jalurmasuk_id, jalurmasuk, jalurmasukprestasi_id, uid, aktif, ts, tahun_ijazah, tahunskhun, kebutuhan_khusus_chk, jenis_tinggal, kebutuhan_khusus_chk1, kebutuhan_khusus, citacita, citacita_lain, tahunayah, kodeposortu, butuhkhususketayah, tahunibu, kodeposibu, butuhkhususibu, butuhkhususketibu, tahunwali, jarak_km, waktutempuh, waktutempuh_menit, almayah, butuhkhususayah, almibu, alamatibu, alumni, idgugus) values ('$nik', '$idangkatan', '$idangkatan1', '$departemen', '$departemen_sekolah_id', '$nama', '$panggilan', '$tglmasuk', '$kelamin', '$tmplahir', '$tgllahir', '$agama', '$warga', '$jsaudara', '$desa_kode', '$kecamatan_kode', '$kota_kode', '$provinsi_kode', '$alamatsiswa', '$rt_siswa', '$rw_siswa', '$dusun', '$desa', '$kecamatan', '$kodepossiswa', '$jenistinggal', '$alamatortu', '$telponsiswa', '$hpsiswa', '$emailsiswa', '$telponortu', '$hportu', '$hpibu', '$transportasi_kode', '$kps', '$nokps', '$kip', '$nokip', '$namakip', '$nokks', '$no_akte_lahir', '$transportasi_lain', '$jaraksekolah', '$kesekolah', '$berat', '$tinggi', '$kesehatan', '$darah', '', '$kelainan', '$asalsekolah_id', '$kota_asalsekolah', '$tglijazah', '$noijazah', '$tglskhun', '$skhun', '$noujian', '$nisnasal', '$nik_ayah', '$namaayah', '$nik_ibu', '$namaibu', '$tmplahirayah', '$tgllahirayah', '$tempat_bekerja_ayah', '$tmplahiribu', '$tgllahiribu', '$pekerjaanayah', '$pekerjaanibu', '$penghasilanayah_kode', '$penghasilanayah', '$penghasilanibu_kode', '$penghasilanibu', '$pendidikanayah', '$pendidikanibu', '$wnayah', '$wnibu', '$nik_wali', '$wali', '$tmplahirwali', '$tgllahirwali', '$pendidikanwali', '$pekerjaanwali', '$penghasilanwali_kode', '$penghasilanwali', '$tempat_bekerja_wali', '$alamatwali', '$hpwali', '$hubungansiswa', '$pekerjaanayah_lain', '$pekerjaanibu_lain', '$tempat_bekerja_ibu', '$pekerjaanwali_lain', '$rombel_id', '$nama_bank', '$no_rekening_bank', '$nama_pemilik_bank', '$pip', '$alasan_pip', '$idminat', '$jalurmasuk_id', '$jalurmasuk', '$jalurmasukprestasi_id', '$uid', '$aktif', '$dlu', '$tahun_ijazah', '$tahunskhun', '$kebutuhan_khusus_chk', '$jenis_tinggal', '$kebutuhan_khusus_chk1', '$kebutuhan_khusus', '$citacita', '$citacita_lain', '$tahunayah', '$kodeposortu', '$butuhkhususketayah', '$tahunibu', '$kodeposibu', '$butuhkhususibu', '$butuhkhususketibu', '$tahunwali', '$jarak_km', '$waktutempuh', '$waktutempuh_menit', '$almayah', '$butuhkhususayah', '$almibu', '$alamatibu', '2', '$idgugus')";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
					}
				}
	...
	//-----insert deskripsi raport P5
	function insert_deskripsi_raport_p5(){
		$dbpdo = DB::create();
		...
		try {
			...
			$departemen			=	$_POST["departemen"];
			$departemen_sekolah_id=	numberreplace($_POST["departemen_sekolah_id"]);
			$tingkat_id			=	numberreplace($_POST["idtingkat"]);
			$semester_id		=	numberreplace($_POST["semester_id"]);
			$tahunajaran_id 	=	numberreplace($_POST["tahunajaran_id"]);
			$guru_id			=	numberreplace($_POST["guru_id"]);
			$tema				=	petikreplace($_POST["tema"]);
			$aktif				=	numberreplace($_POST["aktif"]);
			$uid				=	$_SESSION["loginname"];
			$dlu				=	date("Y-m-d H:i:s");

			$sqlstr = "insert into deskripsi_raport_p5 (departemen, departemen_sekolah_id, tingkat_id, semester_id, tahunajaran_id, guru_id, tema, aktif, uid, dlu) values ('$departemen', '$departemen_sekolah_id', '$tingkat_id', '$semester_id', '$tahunajaran_id', '$guru_id', '$tema', '$aktif', '$uid', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
			...
			$results			=	$dbpdo->query($sqlstr);
			$data 				=  	$results->fetch(PDO::FETCH_OBJ);
			$deskripsi_p5_id	=	$data->last_id;

			$jmldata = $_POST["jmldata"];
			for($i=0; $i<$jmldata; $i++) {
				$dimensi 		=	petikreplace($_POST['dimensi_'.$i.'']);

				if($dimensi != "") {
					$line = maxline('deskripsi_raport_p5_dimensi', 'line', 'deskripsi_p5_id', $deskripsi_p5_id, '');

					$sqlstr = "insert into deskripsi_raport_p5_dimensi (deskripsi_p5_id, dimensi, line) values ('$deskripsi_p5_id', '$dimensi', '$line')";
					$sql=$dbpdo->prepare($sqlstr);
					$sql->execute();
				}
			}
	...
	function insert_raport_p5(){	
		...
		try {
			...
			$departemen			=	$_POST["departemen"];
			$departemen_sekolah_id=	numberreplace($_POST["departemen_sekolah_id"]);
			$idtahunajaran	=	numberreplace($_POST["tahunajaran_id"]);
			$idtingkat		=	numberreplace($_POST["idtingkat"]);
			$idkelas		=	numberreplace($_POST["idkelas"]);
			$idsemester		=	numberreplace($_POST["semester_id"]);
			$idguru			=	numberreplace($_POST["guru_id"]);
			$tema_id		=	numberreplace($_POST["tema_id"]);
			$tema			=	petikreplace($_POST["tema"]);
			$idsiswa__ 		=	$_POST["idsiswa"];
			$semua_siswa	=	numberreplace($_POST["semua_siswa"]);
			$deskripsi_p5_id__ = 0;
			$old_ta 		=	numberreplace($_POST['old_ta']);
			...
			$jmldata = numberreplace($_POST['jmldata']);	
			for ($i=0; $i<$jmldata; $i++) {
				$deskripsi_p5_id = numberreplace($_POST['raport_p5_id_'.$i.'']);
		        $deskripsi_p5_id__ = $deskripsi_p5_id;
    			...
    			if($semua_siswa == 0) {
					...
					if($idsiswa_1 != "") {									
						if(!empty($idsiswa_1)) {
							for($q=0; $q<count($idsiswa_1); $q++) {
								$idsiswa = $idsiswa_1[$q];

								$sqlstr="insert into raport_p5 (departemen, departemen_sekolah_id, idtingkat, idkelas, idtahunajaran, idsemester, idsiswa, deskripsi_p5_id, idguru, tema_id, tema, line, uid, dlu, syscode) values ('$departemen', '$departemen_sekolah_id', '$idtingkat', '$idkelas', '$idtahunajaran', '$idsemester', '$idsiswa', '$deskripsi_p5_id', '$idguru', '$tema_id', '$tema', '$old_ta', '$uid', '$dlu', '$syscode')";
								$sql=$dbpdo->prepare($sqlstr);
								$sql->execute();
								...
								$results		=	$dbpdo->query($sqlstr);
								$data 			=  	$results->fetch(PDO::FETCH_OBJ);
								$raport_p5_id	=	$data->last_id;

								//start dimensi
								$jmldata_dimensi = numberreplace($_POST['jmldata_dimensi_'.$i.'']);
								for($no=0; $no<$jmldata_dimensi; $no++) {
									$raport_p5_dimensi_id =	numberreplace($_POST['raport_p5_dimensi_id_'.$i.$no.'']);
									$pilih = numberreplace($_POST['pilih_'.$i.$no.'']);
									if($pilih == 1) {

										//-----insert raport dimensi
										$line_dimensi = maxline('raport_p5_dimensi', 'line', 'raport_p5_id', $raport_p5_id, '');
										$sqlstr="insert into raport_p5_dimensi (raport_p5_id, dimensi_id, line, syscode	) values ('$raport_p5_id', '$raport_p5_dimensi_id', '$line_dimensi', '$syscode')";
										$sql=$dbpdo->prepare($sqlstr);
										$sql->execute();
										
										//start elemen
										$jmldata_elemen		=	numberreplace($_POST['jmldata_elemen_'.$i.$no.'']);
						        		for($x=0; $x<$jmldata_elemen; $x++) {
						        			$select_elemen 		=	$_POST['select_elemen_'.$i.$no.$x.''];
						        			$raport_elemen_id	=	$_POST['raport_p5_elemen_id_'.$i.$no.$x.''];
						        			$dimensi_id 		=	$raport_p5_dimensi_id;
						        			if($select_elemen == 1) {

						        				//-----insert raport elemen
												$line_elemen = maxline('raport_p5_elemen', 'line', 'dimensi_id', $dimensi_id, '');
												$sqlstr="insert into raport_p5_elemen (raport_p5_id, dimensi_id, elemen_id, line, syscode) values ('$raport_p5_id', '$dimensi_id', '$raport_elemen_id', '$line_elemen', '$syscode')";
												$sql=$dbpdo->prepare($sqlstr);
												$sql->execute();
												...
												$results		=	$dbpdo->query($sqlstr);
												$data 			=  	$results->fetch(PDO::FETCH_OBJ);
												$elemen_id		=	$data->last_id;

												//start subelemen
												$jmldata_subelemen 	 =	$_POST['jmldata_subelemen_'.$i.$no.$x.''];
												for($y=0; $y<$jmldata_subelemen; $y++) {
													$select_subelemen 	=	$_POST['select_subelemen_'.$i.$no.$x.$y.''];
													$subelemen_id 		= 	$_POST['raport_p5_subelemen_id_'.$i.$no.$x.$y.''];
													if($select_subelemen == 1) {
														//-----insert raport subelemen
														$line_subelemen = maxline('raport_p5_nilai', 'line', 'elemen_id', $elemen_id, '');
														$sqlstr="insert into raport_p5_nilai (raport_p5_id, elemen_id, subelemen_id, mulai, sedang, sesuai, sangat, line, syscode) values ('$raport_p5_id', '$elemen_id', '$subelemen_id', 0, 0, 0, 0, '$line_subelemen', '$syscode')";
														$sql=$dbpdo->prepare($sqlstr);
														$sql->execute();
					...
					$sqlstr = "select replid from siswa where aktif=1 and idkelas='$idkelas' order by replid";
					$sqlsiswa=$dbpdo->prepare($sqlstr);
					$sqlsiswa->execute();
					while($datasiswa = $sqlsiswa->fetch(PDO::FETCH_OBJ)) {
						$idsiswa = $datasiswa->replid;
						$sqlstr="insert into raport_p5 (departemen, departemen_sekolah_id, idtingkat, idkelas, idtahunajaran, idsemester, idsiswa, deskripsi_p5_id, idguru, tema_id, tema, uid, dlu, syscode) values ('$departemen', '$departemen_sekolah_id', '$idtingkat', '$idkelas', '$idtahunajaran', '$idsemester', '$idsiswa', '$deskripsi_p5_id', '$idguru', '$tema_id', '$tema', '$uid', '$dlu', '$syscode')";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
						...
						// 	$idsiswa = $idsiswa_1[$q];
							...
							// $sqlstr="insert into raport_p5 (departemen, idtingkat, idkelas, idtahunajaran, idsemester, idsiswa, deskripsi_p5_id, idguru, tema_id, tema, line, uid, dlu, syscode) values ('$departemen', '$idtingkat', '$idkelas', '$idtahunajaran', '$idsemester', '$idsiswa', '$deskripsi_p5_id', '$idguru', '$tema_id', '$tema', '$old_ta', '$uid', '$dlu', '$syscode')";
							// $sql=$dbpdo->prepare($sqlstr);
							// $sql->execute();
			...
			##get line
			/*$sqlstr = "select distinct deskripsi_p5_id, tema from raport_p5 where deskripsi_p5_id='$deskripsi_p5_id__' ";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();
			$rows = $sql->rowCount();
			...
			$sqlstr = "update raport_p5 set line='$line' where departemen='SMA' and idtingkat='$idtingkat' and	idkelas='$idkelas' and idtahunajaran='$idtahunajaran' and idsemester='$idsemester' and deskripsi_p5_id='$deskripsi_p5_id__' and tema='$tema'";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();*/
	...
	//setup periode raport p5
	function insert_setup_periode_raport_p5(){
		
		$dbpdo = DB::create();
		
		try {
			
			$departemen_id 	= numberreplace($_POST["departemen_id"]);
			$departemen_sekolah_id = numberreplace($_POST["departemen_sekolah_id"]);
			$idtahunajaran	=	(empty($_POST["idtahunajaran"])) ? 0 : $_POST["idtahunajaran"];
			$semester_id	=	(empty($_POST["semester_id"])) ? 0 : $_POST["semester_id"];
			$tingkat_id		=	(empty($_POST["tingkat_id"])) ? 0 : $_POST["tingkat_id"];
			...
			$sqlstr = "insert into setup_periode_raport_p5 (departemen_id, departemen_sekolah_id, idtahunajaran, semester_id, tingkat_id, tanggal_ttd, aktif, uid, dlu) values ('$departemen_id', '$departemen_sekolah_id', '$idtahunajaran', '$semester_id', '$tingkat_id', '$tanggal_ttd', '$aktif', '$uid', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();	
	...
	//presensiharian_setup
	function insert_presensiharian_setup(){
		...
		try {
			...
			$departemen 			=	$_POST["departemen"];
			$departemen_sekolah_id 	=	numberreplace($_POST["departemen_sekolah_id"]);
			$idtingkat 				=	numberreplace($_POST["idtingkat2"]);
			$idkelas 				=	numberreplace($_POST["idkelas2"]);
			$jam_masuk				=   date("H:i", strtotime($_POST["jam_masuk"]));
			$jam_terlambat			=   date("H:i", strtotime($_POST["jam_terlambat"]));
			$jam_pulang_cepat		=   date("H:i", strtotime($_POST["jam_pulang_cepat"]));
			$jam_pulang				=   date("H:i", strtotime($_POST["jam_pulang"]));
			$aktif					=	(empty($_POST["aktif"])) ? 0 : $_POST["aktif"];
			$uid					=	$_SESSION["loginname"];
			$dlu					=	date("Y-m-d H:i:s");
			
			$sqlstr = "insert into presensiharian_setup (departemen, departemen_sekolah_id, idtingkat, idkelas,  jam_masuk, jam_terlambat, jam_pulang_cepat, jam_pulang, aktif, uid, dlu) values ('$departemen', '$departemen_sekolah_id', '$idtingkat', '$idkelas', '$jam_masuk', '$jam_terlambat', '$jam_pulang_cepat', '$jam_pulang', '$aktif', '$uid', '$dlu')";
			$sql=$dbpdo->prepare($sqlstr);
			$sql->execute();	
