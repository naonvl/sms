<script type="text/javascript">
	function hapus(id) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('room_booking_view') ?>/xm8r389xemx23xb2378e23/"+id+" ";
		}
	}
	
	function checkout(id) {
		if (confirm('Apakah Anda yakin akan checkout data ini?')) {
			document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('room_booking_view') ?>/chk/"+id+" ";
		}
	}
	
	function uncheckout(id) {
		if (confirm('Apakah Anda yakin akan checkout data ini?')) {
			document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('room_booking_view') ?>/unchk/"+id+" ";
		}
	}
</script>
...
<script type="text/javascript">
	function getfilter() {
		...
		document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('room_booking_view') ?>/"+year+"";
	}
</script>
		...
		<div class="col-xs-12">
            ...
            <?php
				...
				if ($delete == "xm8r389xemx23xb2378e23") {
					...
					$delete2=new delete;
					$delete2->delete_room_registration($segmen4);
			?>
					<div class="alert alert-success">
						<strong>Delete Data successfully</strong>
					</div>
                    
                    <meta http-equiv="Refresh" content="0;url=<?php echo $__folder ?><?php echo obraxabrix('room_booking_view') ?>" />
			<?php
            ...
            <?php
				...
				if ($chk == "chk") {
					...
					$update2=new update;
					$update2->update_room_booking_checkout($segmen4);
			?>
					<div class="alert alert-success">
			...
			<?php
				...
				if ($unchk == "unchk") {
					include 'class/class.update.php';
					$update2=new update;
					$update2->update_room_booking_uncheckout($segmen4);
			?>
					<div class="alert alert-success">
						...
						<div class="form-group">
		                    ...
		                    <div class="col-sm-6">
		                    	 <select id="ref" name="ref" class="chosen-select form-control"  style="max-width: 300px; font-size: 12px;" >
		                          	<option value=""></option>
		                            <?php 
		                                //select_room_booking($ref) 
		                                select_room_registration_memo_filter($year, $ref) 
		                            ?>	
						...
						<table id="dynamic-table" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
									...
									<th><?php if($lng==1) { echo 'From Time'; } else { echo 'dari Jam'; } ?></th>
									<th><?php if($lng==1) { echo 'To Time'; } else { echo 's/d Jam'; } ?></th>
									<th><?php if($lng==1) { echo 'Room Booking'; } else { echo 'Peminjaman Ruang'; } ?></th>
							...
							<tbody>
                                <?php			
									$i = 0;
            						$sql=$select->list_room_booking($ref, "", "", $all, $year);
						            while($row_room_booking=$sql->fetch(PDO::FETCH_OBJ)){
									...
									$j++;
										...
										/*$check_color = "";
										if($row_room_booking->checkout == 1) {
											$check_color = 'style="background-color: #7d0000; color: #ffffff"';	
										}*/
										
										$nama_ruang = "";
										$sqlget = $select->get_room_booking_detail($row_room_booking->ref);
										while($dataget=$sqlget->fetch(PDO::FETCH_OBJ)) {
											if($nama_ruang == "") {
												$nama_ruang = $dataget->name;
											} else {
												$nama_ruang = $nama_ruang . ", " . $dataget->name;
											}
										}
								...
								?>
                                        ...
                                        <tr>
                                            <td><?php echo $i ?></td> 
                                            <td><?php echo $row_room_booking->memo ?></td>
						                    <td><?php echo date("d-m-Y", strtotime($row_room_booking->booked)) ?></td>
						                    <td><?php echo date("H:i", strtotime($row_room_booking->booked)) ?></td>
						                    <td><?php echo date("H:i", strtotime($row_room_booking->booked_finish)) ?></td>
											<td><?php echo $nama_ruang ?></td>
											<td><?php echo $row_room_booking->checkout_uid ?></td>
											<td <?= $style ?>><?= $status ?></td>
											<td><?php echo $row_room_booking->uid ?></td>
											<td align="center">
                                            
                                                <?php if (allowupd('frmreturn_goods')==1 && $status != "Kembali") { ?>
    												<a href="<?php echo $__folder ?><?php echo obraxabrix('room_booking') ?>/<?php echo $row_room_booking->ref ?>/kembali" class="tooltip-success" data-rel="tooltip" title="Kembali">
    													<span class="green">
											...
											<td align="center">
                                                ...
                                                <?php if (allowupd('frmbooking')==1) { ?>
    												<a href="<?php echo $__folder ?><?php echo obraxabrix('room_booking') ?>/<?php echo $row_room_booking->ref ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
    													<span class="green">
    														<i class="ace-icon fa fa-pencil-square-o bigger-120"></i>
    													</span>
    												</a>
                                                ...
                                                <?php if (allowdel('frmbooking')==1) { ?>   
                                            	    &nbsp;
    												<a href="JavaScript:hapus('<?php echo $row_room_booking->ref ?>')" class="tooltip-error" data-rel="tooltip" title="Delete">
    													<span class="red">
