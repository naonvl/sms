...
<script>
	...
	function checklist(lne) {
		var slc = document.getElementById('registration_slc_' + lne).checked;
		
		if (slc) {
			pilih = pilih + '|' + document.getElementById('registration_slc_' + lne).checked;
			/*if(pilih == true) {
				pilih = "1";
			}*/	
			$('#pilih').html('<input type="hidden" size="100" id="registration_slc" name="registration_slc" value="'+ pilih +'" >');
			
			refref = refref + '|' + document.getElementById('room_ref_'  + lne).value;
			$('#ref_id').html('<input type="hidden" size="100" id="room_ref" name="room_ref" value="'+ refref +'" >');
			...
			pilih3 = false; //document.getElementById('registration_slc_' + lne).checked;
			/*if(pilih3 == false) {
				pilih3 = "";
			}*/
			...
			refref3 = document.getElementById('room_ref_'  + lne).value;
	...
	function checklist_update(lne) {
		var slc = document.getElementById('registration_slc_' + lne).value;
		...
		<div class="col-xs-12">
            ...
            <?php 
				...
				if ($ref != "") {
					...
					$checkout = "";
					if($row_registration->checkout == 1) {
						$checkout = "checked";
					}
					$checkout_uid = $row_registration->checkout_uid;
		...
		<div class="row">
			<div class="col-xs-12">
				...
				<form class="form-horizontal" role="form" action="" method="post" name="registration" id="registration" enctype="multipart/form-data" onSubmit="return cekinput('name,memo');" >
					...
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'Date'; } else { echo 'Tanggal Pengajuan'; } ?> *)</label>
						<div class="col-sm-3">
				...
				<div class="form-group">
					...
					<?php if($segmen4 == 'kembali') { ?>
						...
						<div class="form-group">
							<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'Finish Date'; } else { echo 'Tanggal Selesai'; } ?> *)</label>
							<div class="col-sm-3">
								<input type="text" id="checkout_date" name="checkout_date" style="font-size: 12px" class="form-control date-picker" data-date-format="dd-mm-yyyy" autocomplete="off" value="<?php echo $checkout_date ?>">								
							</div>
						</div>

						<div class="form-group">
							<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Nama Penanggungjawab</label>
							<div class="col-sm-3">
								<input type="text" id="checkout_uid" name="checkout_uid" class="form-control" autocomplete="off" value="<?php echo $checkout_uid ?>">
							</div>
						</div>	
						...
						<div class="form-group">
							...
							<div class="col-sm-6">
								<input type="checkbox" name="checkout" id="checkout" class="ace" value="1" <?= $checkout ?> ><span class="lbl"></span>
							</div>
						</div>
	        ...
	        <?php 
              ...
              <?php if (allowdel('frmroom_booking','')==1 && $segmen4 != 'kembali') { ?>
                  &nbsp;
	            ...
	            <?php if (allowadd('frmreturn_booking_room','')==1) { ?>
	              <?php if($segmen4 == 'kembali') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Selesai" />
    						<?php } ?>
              <?php } ?>
