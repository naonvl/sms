	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				include("app/exec/department_insert.php"); 
				
				if ($ref != "") {
					$sql=$select->list_departemen($ref);
					$row_department=$sql->fetch(PDO::FETCH_OBJ);							
				}		
			?>
            
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" action="" method="post" name="department" id="department" enctype="multipart/form-data" onSubmit="return cekinput('name');" >
			
				<input type="hidden" id="id" name="id" value="<?php echo $ref ?>" >
                
                <div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'Department Name'; } else { echo 'Nama Departemen'; } ?> *)</label>
					<div class="col-sm-3">
						<input type="text" id="departemen" name="departemen" class="form-control" style="width: 300px" value="<?php echo $row_department->departemen ?>">
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<select name="nipkepsek" id="nipkepsek" class="chosen-select form-control" style="width:auto;" >
							<option value=""></option>
							<?php select_pegawai($row_department->nipkepsek); ?>
						</select>
					</div>
				</div> */ ?>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<?php if($ref=='') { ?>
							<input class="ace" type="checkbox" name="aktif" id="aktif" value="1" checked />
						<?php } else { ?>
							<input class="ace" type="checkbox" name="aktif" id="aktif" value="1" <?php if($row_department->aktif==1) echo "checked";?>/>
						<?php } ?>
						<span class="lbl"></span>
					</div>
				</div><!-- /.row -->
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<input type="text" id="uid" name="uid" readonly class="form-control" style="width: 300px" value="<?php echo $row_department->info1 ?>">
					</div>
				</div><!-- /.form-group -->
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<input type="text" id="ts" name="ts" readonly class="form-control" style="width: 300px" value="<?php echo $row_department->ts ?>" >
					</div>
				</div><!-- /.form-group -->
              
              	<?php
              		if($ref == "") {
              			include("departemen_sekolah.php");
              		} else {
              			include("departemen_sekolah_update.php");
              		}	
              	?>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowupd('frmdepartment')==1) { ?>
                            <?php if($ref!='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Update" />
    						<?php } ?>
                        <?php } ?>
						
                        <?php if (allowadd('frmdepartment')==1) { ?>
    						<?php if($ref=='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Save" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php if (allowdel('frmdepartment')==1) { ?>
                            &nbsp;
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('department_view') ?>'" />
...
<script type="text/javascript">
	jQuery(function($) {
		...
		for(j=0; j<10; j++) {
			$('#tanggal_awal_'+j).datetimepicker().next().on(ace.click_event, function(){
				$(this).prev().focus();
			});

			$('#tanggal_akhir_'+j).datetimepicker().next().on(ace.click_event, function(){
				$(this).prev().focus();
			});
		}
