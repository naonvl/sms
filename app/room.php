	...
	function cekinput(fid) {  
	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='asset_ref') {
			alert('Gedung cannot empty!');				
		  }
		  
		  if (document.getElementById(arrf[i]).name=='code') {
			alert('Kode Ruang cannot empty!');				
		  }
		  
		  if (document.getElementById(arrf[i]).name=='name') {
			alert('Nama Ruang cannot empty!');				
		  }
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				$ref2 = notran(date('y-m-d'), 'frmroom', '', '', ''); 
					
				include("app/exec/room_insert.php"); 
				
				if ($ref != "") {
					$sql=$select->list_room($ref);
					$row_room=$sql->fetch(PDO::FETCH_OBJ);	
					
                    $ref2 = $row_room->ref;
                    $tanggal_perolehan = $row_room->tanggal_perolehan;
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="room" id="room" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('ref,asset_ref,code,name');" >
				...
				<div class="form-group">
                    ...
                    <div class="col-sm-3">
                    	 <select id="asset_ref" name="asset_ref" class="chosen-select form-control"  style="max-width: 300px; font-size: 12px;" >
                    	 	<option value=""></option>
                          	<?php 
                                select_asset_gedung($row_room->asset_ref) 
                            ?>	
				...
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Kode Ruang *)</label>
					<div class="col-lg-3">
						<input type="text" id="code" name="code" class="form-control" value="<?php echo $row_room->code ?>">
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<input type="text" id="name" name="name" class="form-control" value="<?php echo $row_room->name ?>">
					</div>
				</div>
                
                <div class="form-group">
					<label for="text2" class="col-sm-3 control-label no-padding-right">Penanggung Jawab</label>
					<div class="col-sm-5">
						<select id="employee_id" name="employee_id" class="chosen-select form-control" style="height: 35px">
							<option value=""></option>
							<?php combo_select_active("pegawai","replid","nama","aktif","1",$row_room->employee_id) ?>
						</select>
					</div>
				</div>
				
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Peminjaman Ruang</label>
					<div class="col-sm-3">
						<?php if($ref=='') { ?>
							<input type="checkbox" name="booking" id="booking" class="ace" value="1" /><span class="lbl"></span>
						<?php } else { ?>
							<input type="checkbox" name="booking" id="booking" class="ace" value="1" <?php if($row_room->booking==1) echo "checked";?>/><span class="lbl"></span>
						<?php } ?>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						...
						<?php } else { ?>
							<input type="checkbox" name="active" id="active" class="ace" value="1" <?php if($row_room->active==1) echo "checked";?>/><span class="lbl"></span>
						<?php } ?>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<input type="text" id="uid" name="uid" readonly class="form-control" value="<?php echo $row_room->uid ?>">
					</div>
				</div><!-- /.form-group -->
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<input type="text" id="dlu" name="dlu" readonly class="form-control" value="<?php echo $row_room->dlu ?>" >
					</div>
				</div>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('room_view') ?>'" />
