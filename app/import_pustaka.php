	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='file') {
			alert('File CSV tidak boleh kosong!');				
		  }
		...
		<div class="col-xs-12">
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form class="form-horizontal" role="form" action="" method="post" name="pustaka" id="pustaka" enctype="multipart/form-data" onSubmit="return cekinput('departemen,departemen_sekolah_id,file');">
						...
						<div class="form-group">
							<label class="col-sm-3 control-label no-padding-right" for="form-field-1">File CSV *)</label>
							<div class="col-sm-4">
        ...
        if (preg_match("/,/",$datacol) == 0) {
	        while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
	        	...
	        	if($x > 0) {
					...
					$judul				=	petikreplace($column[2]);
					$abstraksi			=	""; //petikreplace($column[11]);
					$keyword			=	""; //petikreplace($column[9]);
					$tahun				=	numberreplace($column[5]);
					$keteranganfisik	=	""; //petikreplace($column[10]);
					$penulis			=	petikreplace($column[3]);
					$penerbit			=	petikreplace($column[4]);
					$format				=	""; //$column[8];
					$katalog			=	""; //petikreplace($column[4]);
					$kodepustaka_crt	=	$column[1];

					$keterangan			=	""; //petikreplace($column[9]);
					$harga				=	0; //numberreplace($column[3]);
					$jumlah				=	numberreplace($column[7]);
					$departemen			=	$_POST["departemen"];
					$departemen_sekolah_id 	=	$_POST["departemen_sekolah_id"];
					$tanggal_masuk		=	date("Y-m-d"); //, strtotime($column[14]));
					$keterangan_pustaka	=	petikreplace($column[9]);
					$info1				=	petikreplace($column[6]); //ISBN/SSN
					$info2				=	petikreplace($column[8]); //JUMLAH HALAMAN
					$ts					=	date("Y-m-d H:i:s");

					/*--------format---------*/
					$sql="select replid from format where nama='$format'";
					$results=$dbpdo->query($sql);
					$rows=$results->rowCount();
					if($rows == 0) {
						
						$sql="insert into format (kode, nama) values ('$format', '$format')";
						$results=$dbpdo->query($sql);

						$sqlstr 		= 	"select last_insert_id() last_id";
						$results		=	$dbpdo->query($sqlstr);
						$data 			=  	$results->fetch(PDO::FETCH_OBJ);
						$format_id		=	$data->last_id;
					} else {
						$data = $results->fetch(PDO::FETCH_OBJ);
						$format_id = $data->replid;
					}
					//----------------
					
					/*--------penulis---------*/
					if(!empty($penulis)) {
						$sql="select replid from penulis where nama='$penulis'";
						$results=$dbpdo->query($sql);
						$rows=$results->rowCount();
						if($rows == 0) {
							$date	=	date('Y-m-d');    
	    					$kode 	=	notran($date, 'frmpenulis', '', '', ''); //---get no ref

							$sql="insert into penulis (kode, nama) values ('$kode', '$penulis')";
							$results=$dbpdo->query($sql);

							$sqlstr 		= 	"select last_insert_id() last_id";
							$results		=	$dbpdo->query($sqlstr);
							$data 			=  	$results->fetch(PDO::FETCH_OBJ);
							$penulis_id		=	$data->last_id;

							notran($date, 'frmpenulis', 1, '', '') ; //----eksekusi ref
						} else {
							$data = $results->fetch(PDO::FETCH_OBJ);
							$penulis_id = $data->replid;
						}
					} else {
						$penulis_id = 0;
					}
					...
					/*--------penerbit---------*/
					if(!empty($penerbit)) {
						$sql="select replid from penerbit where nama='$penerbit'";
						$results=$dbpdo->query($sql);
						$rows=$results->rowCount();
						if($rows == 0) {
							$date	=	date('Y-m-d');    
	    					$kode 	=	notran($date, 'frmpenerbit', '', '', ''); //---get no ref

							$sql="insert into penerbit (kode, nama) values ('$kode', '$penerbit')";
							$results=$dbpdo->query($sql);

							$sqlstr 		= 	"select last_insert_id() last_id";
							$results		=	$dbpdo->query($sqlstr);
							$data 			=  	$results->fetch(PDO::FETCH_OBJ);
							$penerbit_id		=	$data->last_id;

							notran($date, 'frmpenerbit', 1, '', '') ; //----eksekusi ref
						} else {
							$data = $results->fetch(PDO::FETCH_OBJ);
							$penerbit_id = $data->replid;
						}
					} else {
						$penerbit_id = 0;
					}
					...
					/*--------katalog---------*/
					if(!empty($katalog)) {
						$sql="select replid from katalog where nama='$katalog'";
						$results=$dbpdo->query($sql);
						$rows=$results->rowCount();
						if($rows == 0) {
							$date	=	date('Y-m-d');    
	    					$kode 	=	notran($date, 'frmkatalog', '', '', ''); //---get no ref

							$sql="insert into katalog (kode, nama) values ('$kode', '$katalog')";
							$results=$dbpdo->query($sql);

							$sqlstr 		= 	"select last_insert_id() last_id";
							$results		=	$dbpdo->query($sqlstr);
							$data 			=  	$results->fetch(PDO::FETCH_OBJ);
							$katalog_id		=	$data->last_id;

							notran($date, 'frmkatalog', 1, '', '') ; //----eksekusi ref
						} else {
							$data = $results->fetch(PDO::FETCH_OBJ);
							$katalog_id = $data->replid;
						}
					} else {
						$katalog_id = 0;
					}
					...
					if($judul != "") {
						...
						$sqlstr="select replid from pustaka where judul='$judul'";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
						$rows=$sql->rowCount();
						$dataitem = $sql->fetch(PDO::FETCH_OBJ);
						$lastid		= $dataitem->replid;
											
						if($rows == 0) {
							$sqlstr = "insert into pustaka (judul, abstraksi, keyword, tahun, keteranganfisik, penulis, penerbit, format, katalog, photo, file_ebook, keterangan, harga, info1, info2, info3, departemen, departemen_sekolah_id, tanggal_masuk, keterangan_pustaka, ts) values ('$judul', '$abstraksi', '$keyword', '$tahun', '$keteranganfisik', '$penulis_id', '$penerbit_id', '$format_id', '$katalog_id', '$photo', '$file_ebook', '$keterangan', '$harga', '$info1', '$info2', '', '$departemen', '$departemen_sekolah_id', '$tanggal_masuk', '$keterangan_pustaka', '$ts')";
							$sql=$dbpdo->prepare($sqlstr);
							$sql->execute();
							...
							//GenKodePustaka($katalog_id,$penulis_id,$judul,$format,$counter);
							$sqlstr = "insert into daftarpustaka (pustaka, perpustakaan, kodepustaka) values ('$lastid', '1', '$kodepustaka')";
							$sql=$dbpdo->prepare($sqlstr);
							$sql->execute();

							//update counter catalog
							$sql="update katalog set counter=ifnull(counter,0) + 1 where replid='$katalog_id'";
							$results=$dbpdo->query($sql);
							
							$insert++;
						} else {
							$kodepustaka = $kodepustaka_crt;
							$sqlstr = "select replid from daftarpustaka where kodepustaka='$kodepustaka'";
							$sql=$dbpdo->prepare($sqlstr);
							$sql->execute();
							$rows_ptk = $sql->rowCount();
