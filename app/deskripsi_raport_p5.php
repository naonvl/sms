	...
	function cekinput(fid) {  
	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='tahunajaran_id') {
...
<script type="text/javascript">
	function deskripsi_nilai(replid, deskripsi_p5_dimensi_id) 
	{
		window.open("<?= $__folder ?>app/deskripsi_raport_p5_nilai.php?id="+replid+"&deskripsi_p5_dimensi_id="+deskripsi_p5_dimensi_id,"Find","width=1000,height=600,left=50,top=10,toolbar=0,status=0,scroll=1,scrollbars=yes");
	}
</script>        
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				include("app/exec/insert_deskripsi_raport_p5.php");
				
				$ekstra_sifat = "";
				$aktif = "checked";
				if ($ref != "") {
					$sql=$select->list_deskripsi_raport_p5($ref);
					$deskripsi_raport_p5_data=$sql->fetch(PDO::FETCH_OBJ);	

					$departemen 	=	$deskripsi_raport_p5_data->departemen;
					$departemen_sekolah_id 	=	$deskripsi_raport_p5_data->departemen_sekolah_id;
					$idtingkat = $deskripsi_raport_p5_data->tingkat_id;
				}		
			?>
            
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="deskripsi_raport_p52" id="deskripsi_raport_p52" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('departemen,departemen_sekolah_id,tahunajaran_id,tingkat_id,semester_id');" >
        ...
        <div class="form-group">
					...
					<div class="col-sm-3">
						<select name="departemen" id="departemen" class="chosen-select form-control" onchange="loadHTMLPost2('<?php echo $__folder ?>app/raport_p5_ajax.php','sekolah_id','getsekolah_tingkat','departemen','__folder')" >
							<option value=""></option>
        ...
        <div class="form-group">
					...
					<div class="col-sm-3">
						<select name="tahunajaran_id" id="tahunajaran_id" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_thnajaran_all($deskripsi_raport_p5_data->tahunajaran_id) ?>
						</select>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-3" id="tingkat_id">						
						<select name="idtingkat" id="idtingkat" class="chosen-select form-control" onchange="loadHTMLPost2('<?php echo $__folder ?>app/general_ajax.php','kelas_id','getkelas_departemen_sekolah','idtingkat','__folder')" >
							<option value=""></option>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">						
						<select name="semester_id" id="semester_id" class="chosen-select form-control" >
							...
							<?php 
								$departemen = "SMA";
								select_semester_all($departemen, $deskripsi_raport_p5_data->semester_id) 
							?>
						</select>						
					</div>
				</div>
				...
				<?php
					if($ref == "") {
						include("deskripsi_raport_p5_detail.php");
					} else {
						include("deskripsi_raport_p5_detail_update.php");
					}
        		?>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowupd('frmdeskripsi_raport_p5')==1) { ?>
                            <?php if($ref!='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Update" />
    						<?php } ?>
                        <?php } ?>
						
                        <?php if (allowadd('frmdeskripsi_raport_p5')==1) { ?>
    						<?php if($ref=='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Simpan" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php if (allowdel('frmdeskripsi_raport_p5')==1) { ?>
                            &nbsp;
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('deskripsi_raport_p5_view') ?>'" />
