...
<script>
	...
	function getpenerimaan() {
		...
		if(tnis != "") {
			document.location.href = "presensipelajaran.php?nis="+tnis;
		} else {
			document.location.href = "presensipelajaran.php";
		}
...
$sqlstr = "select a.replid idsiswa, a.idkelas, a.nama, a.alamatsiswa, b.tingkat, c.kelas, b.departemen from siswa a left join kelas c on a.idkelas=c.replid left join tingkat b on c.idtingkat = b.replid where a.nis = '$nis'";
...
if ( $nis != "") 
{
	...
	$sqlcek 	= "select replid from presensipelajaran where tanggal = '$tanggal' and jam = '$jam' and idkelas = '$idkelas' and idsemester='$idsemester' ";	
	...
	if($rowsph == 0) {
		...
		if($numrowssiswa > 0) {
			...
			//get pelajaran from jadwal
			$sqlstr = "select a.idpelajaran, a.nipguru, date_format(b.jam1, '%H:%i') jam1 from jadwal a left join jam b on a.idjam1=b.replid where date_format(b.jam1, '%H:%i') <= '$jam' and a.idkelas='$idkelas' order by date_format(b.jam1, '%H:%i') desc limit 1";
			$sqljdl=$dbpdo->prepare($sqlstr);
			$sqljdl->execute();
			$data_jadwal = $sqljdl->fetch(PDO::FETCH_OBJ);
			$idpelajaran = numberreplace($data_jadwal->idpelajaran);
			$gurupelajaran = $data_jadwal->nipguru;
			$keterlambatan = 0;
			$jumlahjam = 0;
			
			$sqlins		= "insert into presensipelajaran (idkelas, idsemester, idpelajaran, tanggal, jam, gurupelajaran, keterangan, materi, objektif, refleksi, rencana, keterlambatan, jumlahjam) values ('$idkelas', '$idsemester', '$idpelajaran', '$tanggal', '$jam', '$gurupelajaran', '$keterangan', '$materi', '$objektif', '$refleksi', '$rencana', '$keterlambatan', '$jumlahjam')";
...
<tr height="150">
	...
	<table border="0" align="center" cellspacing="0" cellpadding="5">
		...
		<tr>
			<td colspan="3" align="center"><strong>Masukkan NIS</strong></td>
