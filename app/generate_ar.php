	...
	function cekinput(fid) {  
	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='finance_type') {
			alert('Jenis Iuran tidak boleh kosong!');				
		  }
...
<div class="page-content">
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="generate_ar" id="generate_ar" enctype="multipart/form-data" onSubmit="return cekinput('departemen,departemen_sekolah_id,idtingkat,idangkatan,tahunbuku,finance_type');" >
				
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Unit *)</label>
					<div class="col-sm-3">
						<select name="departemen" id="departemen" class="chosen-select form-control" onchange="loadHTMLPost2('../app/general_ajax.php','sekolah_id','get_sekolah_tingkat','departemen')" >
							<option value=""></option>
				...
				<div class="form-group">
					...
					<div class="col-sm-4" id="sekolah_id">
						<select name="departemen_sekolah_id" id="departemen_sekolah_id" class="chosen-select form-control" onchange="loadHTMLPost2('../app/general_ajax.php','tingkat_id','get_tingkat_sekolah','departemen_sekolah_id')">
							<option value=""></option>
				...
				<div class="form-group">
					...
					<div class="col-sm-3" id="tingkat_id">
						<select name="idtingkat" id="idtingkat" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_tingkat_sekolah($departemen_sekolah_id, $idtingkat); ?>
						</select>								
					</div>
				</div>
				...
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Tahun Buku *)</label>
					<div class="col-sm-3">
						<select name="tahunbuku" id="tahunbuku" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_tahunbuku($tahunbuku); ?>
						</select>								
					</div>
				</div>

				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Jenis Iuran *)</label>
					<div class="col-sm-3">
						<select name="finance_type" id="finance_type" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_finance_type("in", "siswa", $finance_type); ?>
						</select>								
					</div>
				</div>

				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Total Iuran</label>
					<div class="col-sm-3">
						<input type="text" id="amount" name="amount" onkeyup="formatangka('amount')" autocomplete="off" class="form-control" style="text-align: right" value="">							
					</div>
				</div>
			...
			if($_POST["submit"]) {
				...
				$total			=	numberreplace($_POST['amount']);
				...
				//$sqlcln = $selectview->list_siswa('', '', '', '', '', '', $departemen, '', '', '', '0', $idangkatan, '', $departemen_sekolah_id);
				$sqlcln = $selectview->list_siswa('', '', '', '', '', '', $departemen, '', '', '', '0', '', '', $departemen_sekolah_id='');
				while($datacln = $sqlcln->fetch(PDO::FETCH_OBJ)) {
					
					//get bulan dalam setahun
					for($b=1; $b<=12; $b++) {
						
						$departemen_sekolah_id = 	numberreplace($datacln->departemen_sekolah_id);
						$year			=	$tahunbuku;
						$year_month 	= 	$year.'-'.$b;
						$year_month 	= 	date('Y-m', strtotime($year_month . '+6 months'));
						
						$contact_code	=	$datacln->replid;
						$ref			=	$finance_type.'-'.$contact_code.'-'.$year_month;
						
						$date			=	'01'; //date('d', strtotime($datacln->dlu));
						...
						$due_date		=	$tahun.'-'.$bulan.'-'.$hari;
						$due_date		=	date('Y-m-d', strtotime($due_date));
						
						$client_code	=	$datacln->replid;
						$currency_code	=	'';
						$rate			=	0;
						$top			=	'';

						$sqlstr = "select name from finance_type where id='$finance_type'";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
						$data_fnc = $sql->fetch(PDO::FETCH_OBJ);						
						$memo			=	$data_fnc->name;
						$line			=	$b;
						...
						$sqlstr = "select ref from ar where ref='$ref' and contact_code='$contact_code' and date_format(date, '%Y-%m-%d')='$date'";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();
						$rows = $sql->rowCount();
						...
						if( $rows == 0 && $total > 0 ) {
							...
							$sqlstr="insert into ar(ref, invoice_no, date, due_date, contact_type, contact_code, contact_other, departemen, departemen_sekolah_id, debit_amount, credit_amount, invoice_type, ref_type, currency_code, rate, exchange_type, exchange_date, top, description, uid, dlu, line) values('$ref', '$ref', '$date', '$due_date', 'C', '$client_code', '', '$departemen', '$departemen_sekolah_id', '$total', 0, 'iuran_siswa', 'iuran_siswa', '$currency_code', '$rate', '0', '00:00:00', '$top', '$memo', '$uid', '$dlu', '$line')";
							$sqlx=$dbpdo->prepare($sqlstr);
							$sqlx->execute();
							
							if($sqlx) {
								//echo 'NIS : '.$datacln->nis.'; Nama :'.$datacln->nama.'<br>';
								$insert++;
							}
						} else {
							$sqlstr="update ar set debit_amount='$total' where ref='$ref' and line='$line'";
							$sqlx=$dbpdo->prepare($sqlstr);
							$sqlx->execute();
			...
			<table align="center" style="font-size: 25px; color: #07581c">
				<tr>
					<td><?php echo "Jumlah Generate Data : " . $insert; ?></td>
				</tr>
				<tr>
					<td><?php echo "Jumlah Generate Data Terupdate : " . $update; ?></td>
				</tr>
			</table>
