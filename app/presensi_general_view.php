...
<script type="text/javascript">
	function hapus(id) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('presensi_general_view') ?>/xm8r389xemx23xb2378e23/"+id+" ";
		}
	}
</script>
		...
		<div class="col-xs-12">
            ...
            <?php
				...
				if ($delete == "xm8r389xemx23xb2378e23") {
					...
					$delete2=new delete;
					$delete2->delete_presensi_general($segmen4);
			?>
					<div class="alert alert-success">
						<strong>Delete Data successfully</strong>
					</div>
                    
                    <meta http-equiv="Refresh" content="0;url=<?php echo $__folder ?><?php echo obraxabrix('presensi_general_view') ?>" />
			<?php
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form class="form-horizontal" role="form" action="<?php echo $nama_folder ?>/<?php echo obraxabrix('presensi_general_view') ?>" method="post" name="pegawai_list" id="pegawai_list" enctype="multipart/form-data" >
		            	...
		            	<div class="form-group">
							...
							<div class="col-sm-3">
								<input type="text" id="to_tanggal" name="to_tanggal" class="form-control date-picker" data-date-format="dd-mm-yyyy" autocomplete="off" value="<?php echo $to_tanggal ?>">				
							</div>
						</div>

						<div class="form-group">
							<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'All'; } else { echo 'Semua'; } ?></label>
							<div class="col-sm-3">
								<input type="checkbox" id="all" name="all" class="ace" value="1" <?php echo $all2 ?>>	<span class="lbl"></span>			
							</div>
						</div>
					...
					<div>
						<table id="dynamic-table" class="table table-striped table-bordered table-hover" style="font-size: 12px">
							...
							<tbody>
                                <?php			
									$i = 0;
            						$sql=$select->list_presensi_general_view("", $tanggal, $to_tanggal, $all);
						            while($presensi_general_view=$sql->fetch(PDO::FETCH_OBJ)){
								...
								?>
                                        ...
                                        <tr>
                                            <td><?php echo $i ?></td>
                                            <td><?php echo $presensi_general_view->tanggal ?></td>
                                            <td align="center"><?php echo $presensi_general_view->hadir + $presensi_general_view->absen ?></td>
											<td align="center"><?php echo $presensi_general_view->hadir ?></td>
											<td align="center"><?php echo $presensi_general_view->absen ?></td>
											<td align="center">
                                            
                                                <?php if (allowupd('frmpresensi_general')==1) { ?>
    												<a href="<?php echo $__folder ?><?php echo obraxabrix('presensi_general') ?>/<?php echo $presensi_general_view->tanggal ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
    													<span class="green">
                                                ...
                                                <?php if (allowdel('frmpresensi_general')==1) { ?>    
                                                    &nbsp;
    												<a href="JavaScript:hapus('<?php echo $presensi_general_view->tanggal ?>')" class="tooltip-error" data-rel="tooltip" title="Delete">
    													<span class="red">
