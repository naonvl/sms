	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				include("app/exec/insert_siswa_terlambat.php");
				...
				if ($ref != "") {
					$sql=$select->list_siswa_terlambat($ref);
					$siswa_terlambat_data=$sql->fetch(PDO::FETCH_OBJ);	
					
					$tanggal = date("d-m-Y", strtotime($siswa_terlambat_data->tanggal));		
					$jam     = date("H", strtotime($siswa_terlambat_data->jam));
					$menit     = date("i", strtotime($siswa_terlambat_data->jam));			
				}		
			?>
            
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="siswa_terlambat2" id="siswa_terlambat2" class="form-horizontal" enctype="multipart/form-data" onSubmit="return cekinput('tanggal,idguru,idtingkat,idkelas');" >
                ...
                <div class="form-group">
					...
					<div class="col-sm-5">						
						<select name="idsiswa" id="idsiswa" class="chosen-select form-control" onchange="loadHTMLPost3('<?php echo $__folder ?>app/siswa_terlambat_ajax.php','tingkat_kelas_id','gettingkat_kelas','idsiswa')" >
							<option value=""></option>
							<?php select_siswa_kelas($siswa_terlambat_data->idsiswa) ?>
						</select>						
					</div>
				</div>
	                ...
	                <div class="form-group">
						...
						<div class="col-sm-3">
							<select name="idtingkat1" id="idtingkat1" disabled class="chosen-select form-control">
								<option value=""></option>
								<?php select_tingkat_unit("SMA", $siswa_terlambat_data->idtingkat); ?>
							</select>		
							<input type="hidden" name="idtingkat" id="idtingkat" value="<?= $siswa_terlambat_data->idtingkat ?>">		
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-3" id="kelas_id">
							<select name="idkelas1" id="idkelas1" disabled class="chosen-select form-control" >
								<option value=""></option>
								<?php select_kelas($siswa_terlambat_data->idtingkat, $siswa_terlambat_data->idkelas); ?>
							</select>		
							<input type="hidden" name="idkelas" id="idkelas" value="<?= $siswa_terlambat_data->idkelas ?>">	
						</div>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-5">
						<textarea id="alasan" name="alasan" class="form-control"><?php echo $siswa_terlambat_data->alasan ?></textarea>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-5">
						<textarea id="penanganan" name="penanganan" class="form-control"><?php echo $siswa_terlambat_data->penanganan ?></textarea>
					</div>
				</div>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowadd('frmsiswa_terlambat')==1) { ?>
    						<?php if($ref=='') { ?>
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . '/' . obraxabrix('siswa_terlambat_view') ?>'" />
