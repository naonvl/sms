<script type="text/javascript">
	function hapus(id) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('pinjam_alat_view') ?>/xm8r389xemx23xb2378e23/"+id+" ";
		}
	}
	
	function checkout(id) {
		if (confirm('Apakah Anda yakin akan checkout data ini?')) {
			document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('registration_view') ?>/chk/"+id+" ";
		}
	}
	
	function uncheckout(id) {
		if (confirm('Apakah Anda yakin akan checkout data ini?')) {
			document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('registration_view') ?>/unchk/"+id+" ";
		}
	}
</script>
...
<script type="text/javascript">
	function getfilter() {
		...
		document.location.href = "<?php echo $__folder ?><?php echo obraxabrix('booking_view') ?>/"+year+"";
	}
</script>
		...
		<div class="col-xs-12">
            ...
            <?php
				...
				if ($delete == "xm8r389xemx23xb2378e23") {
					...
					$delete2=new delete;
					$delete2->delete_pinjam_alat($segmen4);
			?>
					<div class="alert alert-success">
						<strong>Delete Data successfully</strong>
					</div>
                    
                    <meta http-equiv="Refresh" content="0;url=<?php echo $__folder ?><?php echo obraxabrix('pinjam_alat_view') ?>" />
			<?php
            ...
            <?php
				...
				if ($chk == "chk") {
					...
					$update2=new update;
					$update2->update_registration_checkout($segmen4);
			?>
					<div class="alert alert-success">
						<strong>Checkout Data successfully</strong>
					</div>
                    
                    <meta http-equiv="Refresh" content="0;url=<?php echo $__folder ?><?php echo obraxabrix('registration_view') ?>" />
			<?php
			...
			<?php
				...
				if ($unchk == "unchk") {
					include 'class/class.update.php';
					$update2=new update;
					$update2->update_registration_uncheckout($segmen4);
			?>
					<div class="alert alert-success">
						<strong>Uncheckout Data successfully</strong>
					</div>
				...
				<div class="col-xs-12">
					...
					<form class="form-horizontal" role="form" action="" method="post" name="purchase_view" id="purchase_view" class="form-horizontal" enctype="multipart/form-data" >
						...
						<div class="form-group">
		                    ...
		                    <div class="col-sm-3">
		                    	 <select id="year" name="year" class="chosen-select form-control"  style="max-width: 300px; font-size: 12px;" onchange="getfilter()" >
		                          	<?php 
		                                select_goods_loan_year($year) 
		                            ?>	
						...
						<div class="form-group">
		                    ...
		                    <div class="col-sm-6">
		                    	 <select id="ref" name="ref" class="chosen-select form-control"  style="max-width: 300px; font-size: 12px;" >
		                          	<option value=""></option>
		                            <?php 
		                                select_goods_loan_memo_year($year, $ref) 
		                            ?>	
						...
						<table id="dynamic-table" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
                                    ...
                                    <th><?php if($lng==1) { echo 'Ref. No'; } else { echo 'No. Pinjam'; } ?></th>
                                    <th><?php if($lng==1) { echo 'Date'; } else { echo 'Tanggal'; } ?></th>
                                    <th><?php if($lng==1) { echo 'Date Return'; } else { echo 'Tgl Kembali'; } ?></th>
                                    <th><?php if($lng==1) { echo 'Activity'; } else { echo 'Kegiatan'; } ?></th>
									<th><?php if($lng==1) { echo 'Receiver'; } else { echo 'Peminjam'; } ?></th>
									<th><?php if($lng==1) { echo 'Employee Name'; } else { echo 'Petugas Penerima'; } ?></th>	
									<th><?php if($lng==1) { echo 'Qty'; } else { echo 'Jml Alat'; } ?></th>
			                        <th><?php if($lng==1) { echo 'Status'; } else { echo 'Status'; } ?></th>
									<th><?php if($lng==1) { echo 'Date Last Update'; } else { echo 'Tgl & Jam'; } ?></th>
									<th>Kembali</th>
									<th>Edit/Hapus</th>
							...
							<tbody>
                                <?php			
									$i = 0;
            						$sql=$select->list_pinjam_alat($ref, "", "", $all, $year);
						            while($row_registration=$sql->fetch(PDO::FETCH_OBJ)){
            						
            						$i++;
            											
									$j++;
										
										$date_return = date("d-m-Y", strtotime($row_registration->date_return));
		                                if(date("d-m-Y", strtotime($row_registration->date_return))=='01-01-1970') {
											$date_return = "";
										}

										$status = $row_registration->status;
										$style = "";
		                                if($status == "C") {
											$status = "Kembali";
											$style = 'style="color: #1fe01f; font-weight: bold;"';
										} else {
											$status = "Pinjam";
								...
								?>
                                        ...
                                        <tr>
                                            <td><?php echo $i ?></td>
                                            <td><?php echo $row_registration->ref ?></td> 
                                            <td><?php echo date("d-m-Y", strtotime($row_registration->date)) ?></td>
                                            <td><?php echo $date_return ?></td>
                                            <td>
                                            	<?php echo $row_registration->memo ?>
                                            </td>
                                            <td><?php echo $row_registration->receiver ?></td>
                                            <td><?php echo $row_registration->employee_id ?></td>
											<td align="right"><?php echo number_format($row_registration->qty,"0",".",",") ?></td>
											<td <?= $style ?>><?= $status ?></td>
											<td><?php echo date("d-m-Y H:i:s", strtotime($row_registration->dlu)) ?></td>
											<td align="center">
                                            
                                                <?php if (allowupd('frmreturn_goods')==1 && $status != "Kembali") { ?>
    												<a href="<?php echo $__folder ?><?php echo obraxabrix('pinjam_alat') ?>/<?php echo $row_registration->ref ?>/kembali" class="tooltip-success" data-rel="tooltip" title="Approve">
    													<span class="green">
                                            ...
                                            <td align="center">
                                                ...
                                                <?php if (allowupd('frmpinjam_alat')==1 && $row_registration->status != "C") { ?>
    												<a href="<?php echo $__folder ?><?php echo obraxabrix('pinjam_alat') ?>/<?php echo $row_registration->ref ?>" class="tooltip-success" data-rel="tooltip" title="Approve">
    													<span class="green">
    														<i class="ace-icon fa fa-pencil-square-o bigger-120"></i>
    													</span>
    												</a>
                                                ...
                                                <?php if (allowdel('frmpinjam_alat')==1 && $row_registration->status != "C") { ?>   
                                            	    &nbsp;
    												<a href="JavaScript:hapus('<?php echo $row_registration->ref ?>')" class="tooltip-error" data-rel="tooltip" title="Delete">
    													<span class="red">
