	...
	function cekinput(fid) {  
	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='tipe_user') {
			alert('Jenis User tidak boleh kosong!');				
		  }
		...
		<div class="col-xs-12">
            ...
            <?php 
				...
				if ($ref != "") {
					...
					$ganti_pwd_no = $row_usr->ganti_pwd_no;
					if($ganti_pwd_no == 1) {
						$ganti_pwd_no = "checked";
					}

					$employee_id = $row_usr->employee_id;
					$usrid	= $row_usr->usrid;
					$pwdori = $row_usr->pwdori;
					if($copy == 'copy') {
						$act = "checked";
						$usrid = '';
						$pwdori = '';
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="usr" id="usr" enctype="multipart/form-data" onSubmit="return cekinput('usrid,tipe_user');" >
                ...
                <input type="hidden" id="__folder" name="__folder" value="<?php echo $__folder ?>" >
                                        
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> User ID *) </label>

					<div class="col-sm-9" id="user_id">
						<?php if($ref == "" || $copy == "copy" ) { ?>
							<input type="text" id="usrid" name="usrid" placeholder="User ID" class="col-xs-10 col-sm-3" value="<?php echo $usrid ?>" onblur="loadHTMLPost4('<?php echo $__folder ?>app/usr_ajax.php','user_id','cekuser','id','usrid','__folder')">
						<?php } else { ?>
							<input type="text" id="usrid" name="usrid" placeholder="User ID" class="col-xs-10 col-sm-3" value="<?php echo $usrid ?>" onblur="loadHTMLPost4('<?php echo $__folder ?>app/usr_ajax.php','user_id','cekuser_update','id','usrid','__folder')">
						<?php } ?>
					</div>
				</div>
                ...
                <div class="form-group">
					...
					<div class="col-sm-9">
						<input type="password" id="pwd" name="pwd" placeholder="Password" class="col-xs-10 col-sm-3" value="<?php echo $pwdori ?>" />
					</div>
				</div>
                
                <div class="form-group">
                    <label class="col-sm-3 control-label no-padding-right" for="form-field-2"> Data User </label>
                    <div class="col-sm-5">
						<select id="idpegawai" name="idpegawai" data-placeholder="..." class="chosen-select form-control" style="width: auto">
	                        <option value=""></option>
	                        <?php 
	                        	select_pegawai_siswa($row_usr->tipe_user, $idpegawai)
	                        ?>	
				...
				<div class="form-group">
                    ...
                    <div class="col-sm-3">
						<select id="tipe_user" name="tipe_user" data-placeholder="..." class="chosen-select form-control" style="width: auto">
	                        <option value=""></option>
	                        <?php 
	                        	select_user_bagian($row_usr->tipe_user)
	                        ?>	
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<select name="departemen" id="departemen" class="chosen-select form-control" >
							<option value=""></option>
							<?php select_departemen($row_usr->departemen); ?>
						</select>
					</div>
				</div>
                ...
                <div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Tidak Boleh ganti Password </label>

					<div class="col-sm-3">
						<input type="checkbox" name="ganti_pwd_no" id="ganti_pwd_no" class="ace" style="width:70px; height:16px; " value="1" <?php echo $ganti_pwd_no ?> ><span class="lbl"></span>
					</div>
				</div>
                ...
                <div class="form-group">
					...
					<div class="col-sm-3">
						<input type="checkbox" name="active" id="active" class="ace" style="width:70px; height:16px; " value="1" <?php echo $active ?> ><span class="lbl"></span>
					</div>
				</div>
                ...
                <div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                    	...
                    	<?php if($copy != 'copy') { ?>
	                        ...
	                        <?php } ?>
	                    <?php } else { ?>
	                    	<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Copy" />
	                    <?php } ?>
                ...
                <?php 
					if($ref == "") {
                        ...
                        include("usr_departemen.php");
					} else {
						include("usr_detail_update.php");
                        include("usr_detail2_update.php");
                        include("usr_departemen_update.php");					}
				?>
